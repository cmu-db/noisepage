            ...
            parallel {
                ...
                stage('ubuntu-20.04/gcc-9.3 (Debug/ASAN/jumbotests)') {
                    ...
                    steps {
                        ...
                        sh script: 'cd build && timeout 20m python3 ../script/testing/junit/run_junit.py --build-type=debug --query-mode=extended -a="pipeline_metrics_enable=True" -a="pipeline_metrics_interval" -a="counters_enable=True" -a="query_trace_metrics_enable=True"', label: 'UnitTest (Extended with pipeline metrics, counters, and query trace metrics)'
                    }
                    post {
                        always {
                            archiveArtifacts(artifacts: 'build/Testing/**/*.xml', fingerprint: true)
