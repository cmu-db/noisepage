...
namespace terrier::storage::index {
    ...
    TEST_F(IndexPopulatorTest, BasicCorrectnessTest) {
        ...
        {
            auto col_idx = table->ColNameToIndex("name");
            ...
            {
                char ss[100];
                sprintf(ss,"%d, %d",col_idx,col.GetAttrSize());
                LOG_INFO(ss)
            }
        }
        ...
        {
            auto col_idx = table->ColNameToIndex("id");
            auto col_oid = catalog::indexkeycol_oid_t(!table->ColNumToOid(col_idx));
            key_cols_0.emplace_back(IndexKeyColumn(col_oid, type::TypeId::INTEGER, false));
            {
                char ss[100];
                sprintf(ss,"%d",col_idx);
                LOG_INFO(ss)
            }
        }
        ...
        {
            auto col_idx = table->ColNameToIndex("address");
        ...
        {
            auto col_idx = table->ColNameToIndex("sex");
            auto col_oid = catalog::indexkeycol_oid_t(!table->ColNumToOid(col_idx));
            key_cols_0.emplace_back(IndexKeyColumn(col_oid, type::TypeId::BOOLEAN, false));
            {
                char ss[100];
                sprintf(ss,"%d",col_idx);
                LOG_INFO(ss)
