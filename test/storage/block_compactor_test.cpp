...
TEST(BlockCompactorTest, SimpleTest) {
  ...
  auto id_bitmap = std::make_shared<arrow::Buffer>(
      reinterpret_cast<uint8_t *>(accessor.ColumnNullBitmap(block, storage::col_id_t(2))), 1);
  auto id_data = std::make_shared<arrow::Buffer>(
      reinterpret_cast<uint8_t *>(accessor.ColumnStart(block, storage::col_id_t(2))), 32);
  auto id_array_data = arrow::ArrayData::Make(arrow::int64(), arrow_metadata.NumRecords(), {id_bitmap, id_data},
  ...
  auto varlen_bitmap = std::make_shared<arrow::Buffer>(
      reinterpret_cast<uint8_t *>(accessor.ColumnNullBitmap(block, storage::col_id_t(1))), 1);
  ...
  auto varlen_array_data =
      arrow::ArrayData::Make(arrow::utf8(), arrow_metadata.NumRecords(), {varlen_bitmap, varlen_offset},
                             {varlen_values_array_data}, arrow_metadata.NullCount(storage::col_id_t(1)));
  auto varlen_array = arrow::MakeArray(varlen_array_data);

  std::vector<std::shared_ptr<arrow::Field>> schema_vector = {arrow::field("id", arrow::int64()),
                                                              arrow::field("varlen", arrow::utf8())};
