...
TEST_F(DataTableTests, SimpleVersionChainTest) {
  ...
  for (uint16_t iteration = 0; iteration < num_iterations; ++iteration) {
    ...
    for (uint32_t i = 0; i < num_inserts; ++i) {
      ...
      undo_buffers[i] = undo_buffer;
      storage::DeltaRecord *undo =
          storage::DeltaRecord::InitializeDeltaRecord(nullptr, VALUE_OF(timestamp_t, 1ull), layout, col_ids, undo_buffer);
    ...
    for (uint32_t i = 0; i < num_inserts; ++i) {
      ...
      table.Select(VALUE_OF(timestamp_t, 0ull), inserted_tuples[i].first, select_row);
