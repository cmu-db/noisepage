...
TEST_F(StorageUtilTests, CopyToProjectedRow) {
  for (uint32_t iteration = 0; iteration < num_iterations_; ++iteration) {
    ...
    storage::ProjectedRowInitializer update_initializer(
        storage::ProjectedRowInitializer::PreparePRInit(layout, update_col_ids));
...
TEST_F(StorageUtilTests, ApplyDelta) {
  for (uint32_t iteration = 0; iteration < num_iterations_; ++iteration) {
    ...
    storage::ProjectedRowInitializer initializer(storage::ProjectedRowInitializer::PreparePRInit(layout, all_col_ids));
    ...
    storage::ProjectedRowInitializer rand_initializer(
        storage::ProjectedRowInitializer::PreparePRInit(layout, rand_col_ids));
