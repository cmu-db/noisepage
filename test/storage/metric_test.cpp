...
TEST_F(MetricTests, TransactionMetricBasicTest) {
  ...
  for (uint8_t i = 0; i < num_iterations_; i++) {
    ...
    for (uint8_t j = 0; j < num_txns_; j++) {
      ...
      txn_manager_->Commit(txn, TestCallbacks::EmptyCallback, nullptr);
      auto latency = static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::milliseconds>(
    ...
    for (auto &raw_data : result) {
      if (raw_data->GetMetricType() == storage::metric::MetricType::TRANSACTION) {
        for (uint8_t j = 0; j < num_txns_; j++) {
          ...
          auto read_cnt = dynamic_cast<storage::metric::TransactionMetricRawData *>(raw_data.get())
                                ->GetTupleRead(txn_id);
          auto update_cnt = dynamic_cast<storage::metric::TransactionMetricRawData *>(raw_data.get())
                               ->GetTupleUpdate(txn_id);
          auto insert_cnt = dynamic_cast<storage::metric::TransactionMetricRawData *>(raw_data.get())
                                ->GetTupleInsert(txn_id);
          auto delete_cnt = dynamic_cast<storage::metric::TransactionMetricRawData *>(raw_data.get())
                               ->GetTupleDelete(txn_id);
          auto latency = dynamic_cast<storage::metric::TransactionMetricRawData *>(raw_data.get())
                               ->GetLatency(txn_id);
