...
//    const auto key_size = std::accumulate(metadata.GetAttributeSizes().begin(), metadata.GetAttributeSizes().end(),
//    0);
//
...
//   */
//  template <typename Random>
//  bool ModifyRandomColumn(const IndexMetadata &metadata, storage::ProjectedRow *pr, byte *reference, float
//  probability,
//                          Random *generator) {
...
//}
//
//// Verify basic key construction and value setting and comparator, equality, and hash operations for various key
/// sizes. / NOLINTNEXTLINE
...
//  EXPECT_FALSE(generic_lt64(key1, key2));
//
//  data = VarlenEntry::CreateInline(reinterpret_cast<byte *>(johnathan),
//  static_cast<uint32_t>(std::strlen(johnathan))); *reinterpret_cast<VarlenEntry *>(pr->AccessForceNotNull(0)) = data;
//  key1.SetFromProjectedRow(*pr, metadata);
//
