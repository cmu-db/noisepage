...
SSBQuery::SSBMakeExecutableQ1(const std::unique_ptr<catalog::CatalogAccessor> &accessor,
...
{
auto d_datekey = expr_maker.CVE(d_schema.GetColumn("d_datekey").Oid(), type::TypeId::DECIMAL);
auto d_year = expr_maker.CVE(d_schema.GetColumn("d_year").Oid(), type::TypeId::DECIMAL);
