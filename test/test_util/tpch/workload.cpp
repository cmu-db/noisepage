...
Workload::Workload(common::ManagedPointer<DBMain> db_main, const std::string &db_name, const std::string &table_root,
  ...
  execution::exec::ExecutionContext exec_ctx{
      ...
      nullptr, common::ManagedPointer<catalog::CatalogAccessor>(accessor_),   exec_settings_};
...
void Workload::GenerateTPCHTables(execution::exec::ExecutionContext *exec_ctx, const std::string &dir_name) {
  // TPCH table names;
  EXECUTION_LOG_INFO("begin read table");
...
void Workload::Execute(int8_t worker_id, uint64_t execution_us_per_worker, uint64_t avg_interval_us, uint32_t query_num,
  ...
  auto total_query_num = queries_ptr_.size();
...
void Workload::MakeExecutableQ4() {
  execution::compiler::test::ExpressionMaker expr_maker;
  // Orders.
  ...
  q4_last_op_ = std::move(order_by);
  auto exec_query = execution::compiler::CompilationContext::Compile(*q4_last_op_.get(), exec_settings_, accessor_.get());
  q4_ = std::move(exec_query);
}

void Workload::MakeExecutableQ5() {
  execution::compiler::test::ExpressionMaker expr_maker;
  // Region.
  auto r_table_oid = accessor_->GetTableOid("region");
  const auto &r_schema = accessor_->GetSchema(r_table_oid);
  // Nation.
  ...
  // Compile plan
  q5_last_op_ = std::move(order_by);
  auto exec_query = execution::compiler::CompilationContext::Compile(*q5_last_op_.get(), exec_settings_, accessor_.get());
  q5_ = std::move(exec_query);
}
void Workload::MakeExecutableQ6(){
  execution::compiler::test::ExpressionMaker expr_maker;
  // Lineitem.
  auto l_table_oid = accessor_->GetTableOid("lineitem");
  const auto &l_schema = accessor_->GetSchema(l_table_oid);
  ...
  // Compile plan
  q6_last_op_ = std::move(agg);
  auto exec_query = execution::compiler::CompilationContext::Compile(*q6_last_op_.get(), exec_settings_, accessor_.get());
  q6_ = std::move(exec_query);
}

void Workload::MakeExecutableQ7() {
  execution::compiler::test::ExpressionMaker expr_maker;
  // Nation.
  auto n_table_oid = accessor_->GetTableOid("nation");
  const auto &n_schema = accessor_->GetSchema(n_table_oid);
  ...
  execution::compiler::test::OutputSchemaHelper l_seq_scan_out{1, &expr_maker};
  {
    ...
    auto extract_year = expr_maker.Function("extractYear", {l_shipdate}, type::TypeId::INTEGER,
                                            terrier::catalog::postgres::EXTRACT_YEAR_PRO_OID);
  ...
  // Compile plan
  q7_last_op_ = std::move(order_by);
  auto exec_query = execution::compiler::CompilationContext::Compile(*q7_last_op_.get(), exec_settings_, accessor_.get());
  q7_ = std::move(exec_query);
}

void Workload::MakeExecutableQ11() {
  execution::compiler::test::ExpressionMaker expr_maker;
  // Supplier.
  auto s_table_oid = accessor_->GetTableOid("supplier");
  const auto &s_schema = accessor_->GetSchema(s_table_oid);
  ...
  // Compile plan
  q11_last_op_ = std::move(order_by);
  auto exec_query = execution::compiler::CompilationContext::Compile(*q11_last_op_.get(), exec_settings_, accessor_.get());
  q11_ = std::move(exec_query);
}


void Workload::MakeExecutableQ18() {
  execution::compiler::test::ExpressionMaker expr_maker;
  // Customer.
  auto c_table_oid = accessor_->GetTableOid("customer");
  const auto &c_schema = accessor_->GetSchema(c_table_oid);
  ...
  // Compile plan
  q18_last_op_ = std::move(order_by);
  auto exec_query = execution::compiler::CompilationContext::Compile(*q18_last_op_.get(), exec_settings_, accessor_.get());
  q18_ = std::move(exec_query);
}
