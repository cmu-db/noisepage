...
namespace terrier::tpcc {
  ...
  WorkloadCached::WorkloadCached(common::ManagedPointer<DBMain> db_main,
  ...
  void WorkloadCached::LoadTPCCQueries(const std::vector<std::string> &txn_names) {
    uint64_t optimizer_timeout = static_cast<uint64_t>(db_main_->GetSettingsManager()->GetInt(settings::Param::task_execution_timeout));
  ...
  void WorkloadCached::InitDelivery() {
    ...
    vec.emplace_back("UPDATE CUSTOMER SET C_BALANCE = C_BALANCE+1, C_DELIVERY_CNT=C_DELIVERY_CNT+1 WHERE C_W_ID=1 AND C_D_ID=2 AND C_ID=3");
  }
  ...
  void WorkloadCached::InitNewOrder() {
    ...
    vec.emplace_back("INSERT INTO \"ORDER\" (O_ID, O_D_ID, O_W_ID, O_C_ID, O_ENTRY_D, O_OL_CNT, O_ALL_LOCAL) VALUES (1,2,3,4,0,6,7)");
    vec.emplace_back("SELECT I_PRICE, I_NAME, I_DATA FROM ITEM WHERE I_ID=1");
    vec.emplace_back("SELECT S_QUANTITY, S_DATA, S_DIST_01, S_DIST_02, S_DIST_03, S_DIST_04, S_DIST_05, S_DIST_06, S_DIST_07, S_DIST_08, S_DIST_09, S_DIST_10 FROM STOCK WHERE S_I_ID=1 AND S_W_ID=2");
    // vec.emplace_back("UPDATE STOCK SET S_QUANTITY = 1, S_YTD = S_YTD + 1, S_ORDER_CNT = S_ORDER_CNT + 1, S_REMOTE_CNT = S_REMOTE_CNT + 1 WHERE S_I_ID = 2 AND S_W_ID = 3");
  }

  void WorkloadCached::InitOrderStatus() {
    auto curr = sqls_.emplace("orderstatus", std::vector<std::string>{});
    std::vector<std::string> &vec = curr.first->second;
    vec.emplace_back("SELECT O_ID, O_CARRIER_ID, O_ENTRY_D FROM \"ORDER\" WHERE O_W_ID=1 AND O_D_ID=2 AND O_C_ID=3 ORDER BY O_ID DESC LIMIT 1");
    vec.emplace_back("SELECT OL_I_ID, OL_SUPPLY_W_ID, OL_QUANTITY, OL_AMOUNT, OL_DELIVERY_D FROM \"ORDER LINE\" WHERE OL_O_ID=1 AND OL_D_ID=2 AND OL_W_ID=3");
    vec.emplace_back("SELECT C_FIRST, C_MIDDLE, C_LAST, C_STREET_1, C_STREET_2, C_CITY, C_STATE, C_ZIP, C_PHONE, C_CREDIT, C_CREDIT_LIM, C_DISCOUNT, C_BALANCE, C_YTD_PAYMENT, C_PAYMENT_CNT, C_SINCE FROM CUSTOMER WHERE C_W_ID=1 AND C_D_ID=2 AND C_ID=3");
    vec.emplace_back("SELECT C_FIRST, C_MIDDLE, C_ID, C_STREET_1, C_STREET_2, C_CITY, C_STATE, C_ZIP, C_PHONE, C_CREDIT, C_CREDIT_LIM, C_DISCOUNT, C_BALANCE, C_YTD_PAYMENT, C_PAYMENT_CNT, C_SINCE FROM CUSTOMER WHERE C_W_ID=1 AND C_D_ID=2 AND C_LAST='page' ORDER BY C_FIRST");
  }
  ...
  void WorkloadCached::InitPayment() {
    ...
    vec.emplace_back("SELECT D_STREET_1, D_STREET_2, D_CITY, D_STATE, D_ZIP, D_NAME FROM DISTRICT WHERE D_W_ID=1 AND D_ID=2");
    vec.emplace_back("SELECT C_FIRST, C_MIDDLE, C_LAST, C_STREET_1, C_STREET_2, C_CITY, C_STATE, C_ZIP, C_PHONE, C_CREDIT, C_CREDIT_LIM, C_DISCOUNT, C_BALANCE, C_YTD_PAYMENT, C_PAYMENT_CNT, C_SINCE FROM CUSTOMER WHERE C_W_ID=1 AND C_D_ID=2 AND C_ID=3");
    vec.emplace_back("SELECT C_DATA FROM CUSTOMER WHERE C_W_ID=1 AND C_D_ID=2 AND C_ID=3");
    vec.emplace_back("UPDATE CUSTOMER SET C_BALANCE = 1, C_YTD_PAYMENT = 2, C_PAYMENT_CNT = 3, C_DATA = '4' WHERE C_W_ID = 1 AND C_D_ID = 2 AND C_ID = 3");
    vec.emplace_back("INSERT INTO HISTORY (H_C_D_ID, H_C_W_ID, H_C_ID, H_D_ID, H_W_ID, H_DATE, H_AMOUNT, H_DATA) VALUES (1,2,3,4,5,0,7,'data')");
    vec.emplace_back("SELECT C_FIRST, C_MIDDLE, C_ID, C_STREET_1, C_STREET_2, C_CITY, C_STATE, C_ZIP, C_PHONE, C_CREDIT, C_CREDIT_LIM, C_DISCOUNT, C_BALANCE, C_YTD_PAYMENT, C_PAYMENT_CNT, C_SINCE FROM CUSTOMER WHERE C_W_ID=1 AND C_D_ID=2 AND C_LAST='page' ORDER BY C_FIRST");
  }
  ...
  void WorkloadCached::InitStockLevel() {
    ...
    vec.emplace_back("SELECT COUNT(DISTINCT S_I_ID) AS STOCK_COUNT FROM \"ORDER LINE\", STOCK WHERE OL_W_ID = 1 AND OL_D_ID = 2 AND OL_O_ID < 3 AND OL_O_ID >= 4 AND S_W_ID = 5 AND S_I_ID = OL_I_ID AND S_QUANTITY < 6");
