...
TEST_F(TpccPlanOrderStatusTests, GetNewestOrd) {
  std::string query = "SELECT O_ID, O_CARRIER_ID, O_ENTRY_D FROM \"ORDER\" WHERE O_W_ID=1 AND O_D_ID=2 AND O_C_ID=3 ORDER BY O_ID DESC LIMIT 1";
  OptimizeQuery(query, "order", tbl_order_, TpccPlanTest::CheckIndexScan);
}

// NOLINTNEXTLINE
TEST_F(TpccPlanOrderStatusTests, GetOrderLine) {
  std::string query = "SELECT OL_I_ID, OL_SUPPLY_W_ID, OL_QUANTITY, OL_AMOUNT, OL_DELIVERY_D "
                      "FROM ORDER_LINE WHERE OL_O_ID=1 AND OL_D_ID=2 AND OL_W_ID=3";
  OptimizeQuery(query, "order_line", tbl_order_line_, TpccPlanTest::CheckIndexScan);
}
