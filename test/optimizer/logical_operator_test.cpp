...
TEST(OperatorTests, LogicalAlterTest) {
  ...
  Operator op1 = LogicalAlter::Make(catalog::table_oid_t(1), {common::ManagedPointer(&cmd1)});
  ...
  Operator op2 = LogicalAlter::Make(catalog::table_oid_t(1), {common::ManagedPointer(&cmd1)});
  EXPECT_TRUE(op1 == op2);
  EXPECT_EQ(op1.Hash(), op2.Hash());
  ...
  Operator op3 = LogicalAlter::Make(catalog::table_oid_t(1), {common::ManagedPointer(&cmd2)});
  EXPECT_FALSE(op1 == op3);
  EXPECT_NE(op1.Hash(), op3.Hash());
}
