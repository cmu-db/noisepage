...
TEST(ValueConditionTests, GetColumnIDTest) {
  auto val = std::make_unique<parser::ConstantValueExpression>(execution::sql::SqlTypeId::Integer, execution::sql::Integer(1));
...
TEST(ValueConditionTests, GetPointerToValueTest) {
  ...
  EXPECT_EQ(parser::ConstantValueExpression(execution::sql::SqlTypeId::Integer, execution::sql::Integer(1)), *v.GetPointerToValue());
}
}  // namespace noisepage::optimizer
