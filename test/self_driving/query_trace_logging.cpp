...
TEST_F(QueryTraceLogging, BasicLogging) {
  ...
  for (size_t j = 0; j < 3; j++) {
    for (size_t i = 0; i < qids.size(); i++) {
      for (int slideDelta = 0; slideDelta < timestamps[i][j]; slideDelta++) {
        size_t time = timestamps[i][j] * metrics::QueryTraceMetricRawData::QUERY_SEGMENT_INTERVAL + slideDelta;
  ...
  auto check_freqs = [util, qids, totals, timestamps](size_t num_interval) {
    ...
    auto freq_check = [&seen, &qid_map](const std::vector<execution::sql::Val *> &values) {
      ...
      qid_map[reinterpret_cast<execution::sql::Integer *>(values[1])->val_]
          ...
          reinterpret_cast<execution::sql::Integer *>(values[3])->val_;

      // Sum the number seen
      seen += reinterpret_cast<execution::sql::Integer *>(values[3])->val_;
    };
...

  {
    ...
    auto func = [&val, qids, texts, db_oids, &parameters](const std::vector<execution::sql::Val *> &values) {
      ...
      NOISEPAGE_ASSERT(
          std::string(reinterpret_cast<execution::sql::StringVal *>(values[2])->StringView()) == texts[qid],
          "Incorrect text recorded");
      ...
      std::string correct = nlohmann::json{type_strs};
...

  {
    util->BeginTransaction();
    size_t row_count = 0;
    auto func = [&row_count, qids](const std::vector<execution::sql::Val *> &values) {
