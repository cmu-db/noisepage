...
class CompilerTest : public TplTest {
 public:
  ...
  static void CompileAndRun(terrier::planner::AbstractPlanNode *node, terrier::transaction::TransactionContext * txn, OutputChecker * checker) {
    // Create the execution context
    OutputStore store{checker, node->GetOutputSchema().get()};
    ...
    exec::ExecutionContext exec_ctx{txn, callack, node->GetOutputSchema().get()};
...
TEST_F(CompilerTest, SimpleSeqScanTest) {
  ...
  CompileAndRun(seq_scan.get(), txn, &multi_checker);
