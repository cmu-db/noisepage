...
TEST_F(CompilerTest, SimpleTest) {
  ...
  auto agg_node = agg_builder.SetOutputSchema(BuildDummyOutputSchema())
...
//  auto *ast = ctx.GetTranslatorOp(*reinterpret_cast<terrier::planner::AggregatePlanNode *>(agg_node_ptr.get()))->Translate();
//  ast::AstDump::Dump(ast);
}
