...
TEST_F(SemaExprTest, ArithmeticOperationTest) {
  SemaExprTestCase tests[] = {
      ...
      {false, "SQL integers should be summable",
       Block({
           DeclStmt(DeclVar("a", "Integer", nullptr)),                                                 // var a: Integer
           DeclStmt(DeclVar("b", "Integer", nullptr)),                                                 // var b: Integer
      ...
      {false, "SQL floats should be summable",
       Block({
           ...
           DeclStmt(DeclVar("b", "Real", nullptr)),                                                    // var b: Real
           DeclStmt(DeclVar("c", BinOp<parsing::Token::Type::PLUS>(IdentExpr("a"), IdentExpr("b")))),  // var c = a + b
       })},
      ...
      {false, "Implicit constant casting in arithmetic is supported",
       Block({
           DeclStmt(DeclVar("a", "float32", nullptr)),  // var a: float32
           DeclStmt(
               DeclVar("b", BinOp<parsing::Token::Type::PLUS>(IdentExpr("a"), FloatLit(23.0)))),  // var b = a + 23.0
       })},
  };
