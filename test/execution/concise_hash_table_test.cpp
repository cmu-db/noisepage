...
TEST_F(ConciseHashTableTest, InsertOverflowTest) {
  ...
  table.Insert(entry_34, entry_34->hash_);
...
TEST_F(ConciseHashTableTest, MultiGroupInsertTest) {
  ...
  table.Insert(entry_97, entry_97->hash_);
  ...
  table.Insert(entry_161, entry_161->hash_);
  ...
  table.Insert(entry_225, entry_225->hash_);
  ...
  // '33' insertion.
  auto *entry_33_v2 = TestEntry(ConciseHashTable::K_MIN_NUM_SLOTS + 33);
  table.Insert(entry_33_v2, entry_33_v2->hash_);
  EXPECT_EQ(34u, entry_33_v2->cht_slot_);
  ...
  auto *entry_33_v3 = TestEntry(ConciseHashTable::K_MIN_NUM_SLOTS + 33);
  table.Insert(entry_33_v3, entry_33_v3->hash_);
  EXPECT_EQ(34u, entry_33_v3->cht_slot_);
}
...
TEST_F(ConciseHashTableTest, CornerCaseTest) {
  ...
  table.Insert(entry_63, entry_63->hash_);
  ...
  table.Insert(entry_62_v2, entry_62_v2->hash_);
...
TEST_F(ConciseHashTableTest, BuildTest) {
  ...
  for (uint32_t i = 1; i < 64; i += 2) {
    ...
    table.Insert(entry, entry->hash_);
