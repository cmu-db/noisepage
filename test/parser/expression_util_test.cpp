...
TEST(ExpressionUtilTest, GetColumnOidsTest) {
  ...
  auto tve = std::make_shared<ColumnValueExpression>(catalog::db_oid_t(1), catalog::table_oid_t(2), catalog::col_oid_t(3));
  ...
  auto op_expr_1 = std::make_shared<OperatorExpression>(ExpressionType::OPERATOR_PLUS, type::TypeId::INVALID, std::move(children1));
  oids.clear();
  ...
  auto op_expr_2 = std::make_shared<OperatorExpression>(ExpressionType::OPERATOR_PLUS, type::TypeId::INVALID, std::move(children2));

  auto children3 = std::vector<std::shared_ptr<AbstractExpression>>{op_expr, op_expr_2};
  auto op_expr_3 = std::make_shared<OperatorExpression>(ExpressionType::OPERATOR_PLUS, type::TypeId::INVALID, std::move(children3));
  oids.clear();
  ExpressionUtil::GetColumnOids(&oids, common::ManagedPointer<AbstractExpression>(op_expr_3.get()));
  EXPECT_THAT(oids, ElementsAre(catalog::col_oid_t(4), catalog::col_oid_t(5), catalog::col_oid_t(6), catalog::col_oid_t(7)));
}
