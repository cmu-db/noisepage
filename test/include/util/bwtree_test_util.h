...
class KeyComparator {
 public:
  inline bool operator()(const long int k1, const long int k2) const { return k1 < k2; }
...
class Envp {
 public:
   ...
   */
  std::string operator()(const std::string &key) const { return Envp::Get(key); }
   ...
   */
  static bool GetValueAsUL(const std::string &key, unsigned long *value_p) {
  ...
  uint64_t Get() {
    if (this->last_n != this->n) {
      if (this->theta > 0. && this->theta < 1.) {
        ...
        this->eta =
            (1. - PowApprox(2. / (double)this->n, 1. - this->theta)) / (1. - Zeta(0, 0., 2, this->theta) / this->zetan);
      }
      this->last_n = this->n;
    ...
    if (this->theta == -1.) {
      ...
      if (uz < 1.) {
        ...
        return (uint64_t)(this->dbl_n * PowApprox(this->eta * (u - 1.) + 1., this->alpha));
      }
    }
...
class CacheMeter {
 private:
  ...
  const char *event_name_list[EVENT_COUNT] = {
      "PAPI_LD_INS", "PAPI_L1_LDM", "PAPI_SR_INS", "PAPI_L1_STM", "PAPI_L3_TCA", "PAPI_L3_TCM",
  };
  ...
  void CheckAllEvents() {
    ...
    for (int i = 0; i < level; i++) {
      if (CheckEvent(event_list[i]) == false) {
        fprintf(stderr, "ERROR: PAPI event %s is not supported\n", event_name_list[i]);
        exit(1);
      }
    }
 ...
 public:
   ...
   * it is false
   */
  CacheMeter(bool start = false, int p_level = 2) : level{p_level} {
  ...
  void Stop() {
    ...
    if (PAPI_stop_counters(counter_list, level) != PAPI_OK) {
      fprintf(stderr, "ERROR: Failed to start counters using PAPI_stop_counters()\n");
      exit(1);
    }
   ...
   */
  std::pair<long long, long long> GetL3CacheUtilization() { return std::make_pair(counter_list[4], counter_list[5]); }

  /*
   * GetL1CacheUtilization() - Returns L1 cache utilizations
   */
  std::pair<long long, long long> GetL1CacheUtilization() {
    return std::make_pair(counter_list[0] + counter_list[2], counter_list[1] + counter_list[3]);
  }
  ...
  void PrintL3CacheUtilization() {
    ...
    std::cout << "    L3 total = " << l3_util.first << "; miss = " << l3_util.second << "; hit ratio = "
              << static_cast<double>(l3_util.first - l3_util.second) / static_cast<double>(l3_util.first) << std::endl;
  ...
  void PrintL1CacheUtilization() {
    ...
    std::cout << "    LOAD/STORE total = " << l1_util.first << "; miss = " << l1_util.second << "; hit ratio = "
              << static_cast<double>(l1_util.first - l1_util.second) / static_cast<double>(l1_util.first) << std::endl;
...
class Permutation {
 ...
 public:
   ...
   */
  inline IntType &operator[](size_t index) { return data[index]; }

  inline const IntType &operator[](size_t index) const { return data[index]; }
};
...
// Benchmark for ART
void BenchmarkARTSeqInsert(ARTType *t, int key_num, int num_thread, long int *array);
void BenchmarkARTSeqRead(ARTType *t, int key_num, int num_thread);
void BenchmarkARTRandRead(ARTType *t, int key_num, int num_thread);
void BenchmarkARTZipfRead(ARTType *t, int key_num, int num_thread);
