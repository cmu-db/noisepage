...
struct Loader {
  ...
  static void PopulateDatabase(transaction::TransactionManager *const txn_manager, Random *const generator,
    ...
    const auto item_tuple_pr_initializer = _->InitializerForProjectedRow(item_tuple_col_oids);
    const auto item_tuple_pr_map = _->ProjectionMapForOids(item_tuple_col_oids);
    ...
    const auto warehouse_tuple_pr_initializer = _->InitializerForProjectedRow(warehouse_tuple_col_oids);
    const auto warehouse_tuple_pr_map = _->ProjectionMapForOids(warehouse_tuple_col_oids);
    ...
    const auto stock_tuple_pr_initializer = _->InitializerForProjectedRow(stock_tuple_col_oids);
    const auto stock_tuple_pr_map = _->ProjectionMapForOids(stock_tuple_col_oids);
    ...
    const auto district_tuple_pr_initializer = _->InitializerForProjectedRow(district_tuple_col_oids);
    const auto district_tuple_pr_map = _->ProjectionMapForOids(district_tuple_col_oids);
    ...
    const auto customer_tuple_pr_initializer = _->InitializerForProjectedRow(customer_tuple_col_oids);
    const auto customer_tuple_pr_map = _->ProjectionMapForOids(customer_tuple_col_oids);
    ...
    const auto history_tuple_pr_initializer = _->InitializerForProjectedRow(history_tuple_col_oids);
    const auto history_tuple_pr_map = _->ProjectionMapForOids(history_tuple_col_oids);

    // Order tuple
    const auto order_tuple_col_oids = Util::AllColOidsForSchema(db->order_schema_);

    const auto order_tuple_pr_initializer = _->InitializerForProjectedRow(order_tuple_col_oids);
    const auto order_tuple_pr_map = _->ProjectionMapForOids(order_tuple_col_oids);
    ...
    const auto new_order_tuple_pr_initializer = _->InitializerForProjectedRow(new_order_tuple_col_oids);
    const auto new_order_tuple_pr_map = _->ProjectionMapForOids(new_order_tuple_col_oids);
    ...
    const auto order_line_tuple_pr_initializer = _->InitializerForProjectedRow(order_line_tuple_col_oids);
    const auto order_line_tuple_pr_map = _->ProjectionMapForOids(order_line_tuple_col_oids);
