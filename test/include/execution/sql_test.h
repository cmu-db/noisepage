...
class SqlBasedTest : public TplTest {
 public:
  ...
  void SetUp() override {
    ...
    // Some of these objects use the loggers in their constructor (I know the catalog does), so they need to be initialized after the loggers.
    TplTest::SetUp();
    ...
    txn_manager_ = std::make_unique<terrier::transaction::TransactionManager>(buffer_pool_.get(), false, log_manager_.get());
  ...
  ~SqlBasedTest() override {
    txn_manager_->Commit(test_txn_, nullptr, nullptr);
    delete test_txn_;
    log_manager_->Shutdown();
  }

  std::unique_ptr<exec::ExecutionContext> MakeExecCtx(exec::OutputCallback && callback = nullptr, const terrier::planner::OutputSchema * schema = nullptr)  {
