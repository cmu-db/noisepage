...
class OutputSchemaHelper {
 public:
  ...
  ExpressionUtil::Expression GetOutput(std::string col_name) {
  ...
  ExpressionUtil::Expression GetGroupByTerm(const std::string & col_name) {
  ...
  ExpressionUtil::AggExpression GetAggTerm(const std::string & agg_name) {
    return aggs[name_to_agg[agg_name]];
  }

  std::shared_ptr<planner::OutputSchema> MakeSchema() {
    return std::make_shared<planner::OutputSchema>(cols_);
  }
 private:
