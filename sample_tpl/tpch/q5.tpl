...
fun setupTables(execCtx: *ExecutionContext, state: *State) -> nil {
  ...
  @tableIterConstructBind(&state.s_tvi, "supplier", execCtx, "s")
...
struct JoinProbe5 {
  lineitem_row : *ProjectedColumnsIterator
  join_row4 : *JoinRow4
}
...
fun checkJoinKey1(execCtx: *ExecutionContext, probe: *ProjectedColumnsIterator, build: *JoinRow1) -> bool {
  if (@pciGetBind(probe, "n", "n_regionkey") != build.r_regionkey) {
    return false
  }
  return true
}
...
fun checkJoinKey5(execCtx: *ExecutionContext, probe: *JoinProbe5, build: *JoinRow5) -> bool {
  // NOTE: this is commented out due to small size of the tables. It's too selective.
  // Uncomment for larger tests
  //if (probe.join_row4.n_nationkey != build.s_nationkey) {
    //return false
  //}
  if (@pciGetBind(probe.lineitem_row, "l", "l_suppkey") != build.s_suppkey) {
    return false
  }
  return true
}
...
fun pipeline1(execCtx: *ExecutionContext, state: *State) -> nil {
  ...
  for (@tableIterAdvance(r_tvi)) {
    ...
    for (; @pciHasNext(vec); @pciAdvance(vec)) {
      // NOTE: This if statement is too selective given the small sample tables. Uncomment it for larger tests.
      //if (@pciGetBind(vec, "r", "r_name") == @stringToSql("ASIA                     ")) {
        // Step 2: Insert into HT1
        var hash_val = @hash(@pciGetBind(vec, "r", "r_regionkey"))
        var build_row1 = @ptrCast(*JoinRow1, @joinHTInsert(&state.join_table1, hash_val))
        build_row1.r_regionkey = @pciGetBind(vec, "r", "r_regionkey")
      //}
    }
  }
  // Step 3: Build HT1
...
fun pipeline2(execCtx: *ExecutionContext, state: *State) -> nil {
  ...
  for (@tableIterAdvance(n_tvi)) {
    ...
    for (; @pciHasNext(vec); @pciAdvance(vec)) {
      // Step 2: Probe HT1
      var hash_val = @hash(@pciGetBind(vec, "n", "n_regionkey"))
      ...
      for (@joinHTIterInit(&hti, &state.join_table1, hash_val); @joinHTIterHasNext(&hti, checkJoinKey1, execCtx, vec);) {
        ...
        var hash_val2 = @hash(@pciGetBind(vec, "n", "n_nationkey"))
        var build_row2 = @ptrCast(*JoinRow2, @joinHTInsert(&state.join_table2, hash_val2))
        build_row2.n_nationkey = @pciGetBind(vec, "n", "n_nationkey")
        build_row2.n_name = @pciGetBind(vec, "n", "n_name")
      }
    }
  }
  // Step 4: Build HT2
...
fun pipeline3(execCtx: *ExecutionContext, state: *State) -> nil {
  ...
  for (@tableIterAdvance(c_tvi)) {
    ...
    for (; @pciHasNext(vec); @pciAdvance(vec)) {
      // Step 2: Probe HT2
      var hash_val = @hash(@pciGetBind(vec, "c", "c_nationkey"))
      var hti: JoinHashTableIterator
      for (@joinHTIterInit(&hti, &state.join_table2, hash_val); @joinHTIterHasNext(&hti, checkJoinKey2, execCtx, vec);) {
        ...
        var hash_val3 = @hash(@pciGetBind(vec, "c", "c_custkey"))
        ...
        build_row3.c_custkey = @pciGetBind(vec, "c", "c_custkey")
      }
    }
  }
  // Step 4: Build HT3
...
fun pipeline4(execCtx: *ExecutionContext, state: *State) -> nil {
  ...
  for (@tableIterAdvance(o_tvi)) {
    ...
    for (; @pciHasNext(vec); @pciAdvance(vec)) {
      if (@pciGetBind(vec, "o", "o_orderdate") >= @dateToSql(1990, 1, 1)
          and @pciGetBind(vec, "o", "o_orderdate") < @dateToSql(2000, 1, 1)) {
        // Step 2: Probe HT3
        var hash_val = @hash(@pciGetBind(vec, "o", "o_custkey"))
        var hti: JoinHashTableIterator
        for (@joinHTIterInit(&hti, &state.join_table3, hash_val); @joinHTIterHasNext(&hti, checkJoinKey3, execCtx, vec);) {
          ...
          var hash_val4 = @hash(@pciGetBind(vec, "o", "o_orderkey"))
          ...
          build_row4.o_orderkey = @pciGetBind(vec, "o", "o_orderkey")
        }
      }
    }
  }
...
fun pipeline5(execCtx: *ExecutionContext, state: *State) -> nil {
  ...
  for (@tableIterAdvance(s_tvi)) {
    ...
    for (; @pciHasNext(vec); @pciAdvance(vec)) {
      ...
      // NOTE: this is commented out due to small size of the tables. It's too selective.
      // Uncomment for larger tests
      var hash_val = @hash(@pciGetBind(vec, "s", "s_suppkey"))    //, @pciGetBind(vec, "s", "s_nationkey"))
      var build_row5 = @ptrCast(*JoinRow5, @joinHTInsert(&state.join_table5, hash_val))
      build_row5.s_suppkey = @pciGetBind(vec, "s", "s_suppkey")
      build_row5.s_nationkey = @pciGetBind(vec, "s", "s_nationkey")
    }
  }
  // Build
  ...
  for (@tableIterAdvance(l_tvi)) {
    ...
    for (; @pciHasNext(vec); @pciAdvance(vec)) {
      ...
      for (@joinHTIterInit(&hti, &state.join_table4, hash_val); @joinHTIterHasNext(&hti, checkJoinKey4, execCtx, vec);) {
        ...
        // NOTE: this is commented out due to small size of the tables. It's too selective.
        // Uncomment for larger tests
        var hash_val5 = @hash(@pciGetBind(vec, "l", "l_suppkey"))              //, join_row4.n_nationkey)
        ...
        for (@joinHTIterInit(&hti5, &state.join_table5, hash_val5); @joinHTIterHasNext(&hti5, checkJoinKey5, execCtx, &join_probe5);) {
          ...
          agg_input.revenue = @pciGetBind(vec, "l", "l_extendedprice") * (1.0 - @pciGetBind(vec, "l", "l_discount"))
