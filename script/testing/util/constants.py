...
OLTP_GIT_CLEAN_COMMAND = "rm -rf {}".format(OLTP_GIT_LOCAL_PATH).split(" ")
OLTP_GIT_COMMAND = "git clone {} {}".format(OLTP_GIT_URL,
                                            OLTP_GIT_LOCAL_PATH).split((" "))
...
OTLP_ANT_COMMAND_BOOTSTRAP = "ant bootstrap -buildfile {}".format(
    OLTP_ANT_BUILD_FILE).split(" ")
OTLP_ANT_COMMAND_RESOLVE = "ant resolve -buildfile {}".format(
    OLTP_ANT_BUILD_FILE).split(" ")
OTLP_ANT_COMMAND_BUILD = "ant build -buildfile {}".format(
    OLTP_ANT_BUILD_FILE).split(" ")
OTLP_ANT_COMMANDS = [
