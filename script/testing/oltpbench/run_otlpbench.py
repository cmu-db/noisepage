...
def config_xml_file(host, port, benchmark, transaction, scalefactor, terminals,
                    time, weights, upload_url, upload_code):
    xml = ElementTree.parse("%ssample_" % (CONFIG_FILES_HOME) + benchmark +
                            "_config.xml")
    root = xml.getroot()
    ...
    root.find("DBUrl").text = "jdbc:postgresql://{0}:{1}/{2}".format(
        host, port, benchmark)  #host, port and benchmark name
...
def start_bench(benchmark, weights, scalefactor):
    ...
    cmd = start_benchmark_script + get_result_path(
        weights, scalefactor) + " 2>&1 | tee results/%s.log" % (
            get_result_path(weights, scalefactor))
    print cmd
    call(cmd, shell=True)
...
def collect_data(result_dir_name, weights, scalefactor):
    ...
    call("cat %s/results/%s.log" %
         (OLTP_HOME, get_result_path(weights, scalefactor)),
         shell=True)
    os.system(
        "mv %s/results/%s.* %s/" %
        (OLTP_HOME, get_result_path(weights, scalefactor), result_dir_name))
...
if __name__ == "__main__":
    ...
    ## EXECUTE
    ## ----------------------------------------------
    config_xml_file(db_host, db_port, benchmark, transaction_isolation,
                    scalefactor, terminals, time, weights, upload_url,
                    upload_code)
    start_bench(benchmark, weights, scalefactor)
    result_dir_name = "tpcc_collected_data_" + weights.replace(
        ",", "_") + "_" + str(scalefactor)
