...
class MicroBenchmarksRunner(object):
    ...
    def run_benchmarks(self, enable_perf):
        ...
        LOG.info("{}/{} benchmarks passed".format(len(self.config.benchmarks) - benchmark_fail_count, len(self.config.benchmarks)))
    ...
    def run_single_benchmark(self, bench_name, enable_perf):
        ...
        if ret_val == 0:
            ...
            LOG.error("Unexpected failure of {BENCHMARK} [ret_val={RET_CODE}]".format(BENCHMARK=bench_name, RET_CODE=ret_val))
            LOG.error(err)
    ...
    def _build_benchmark_cmd(self, bench_name, output_file, enable_perf):
        ...
        cmd = cmd.format(BENCHMARK=benchmark_path, MIN_TIME=self.config.min_time, OUTPUT_FILE=output_file)
...
def is_package_installed(package_name, validation_command='--version'): #TODO: this might cause errors with numactl because it doesn't have a --version but that might not matter cause it will return help command
    """ check to see if package is installed. Throw error if not """
    try:
