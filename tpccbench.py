...
def run_single_benchmark(num_daf_threads, num_worker_threads):
	""" Run benchmark, generate JSON results
	"""
	output_file = "{}daf_{}worker_result.json".format(str(num_daf_threads), str(num_worker_threads))
	...
	os.environ["TERRIER_BENCHMARK_DAF_THREADS"] = str(num_daf_threads) # has to be a str
	...
	print("Executing command [num_worker={}, num_daf={}]: {}".format(num_worker_threads, num_daf_threads, cmd))
...
daf_thread_count = sys.argv[1] 
worker_thread_count = sys.argv[2]

run_single_benchmark(daf_thread_count, worker_thread_count)