...
// TODO(Gus): This is duplicate code, can be simplified by passing in the test parameters as function args
...
BENCHMARK_DEFINE_F(RecoveryBenchmark, HighAbortRate)(benchmark::State &state) {
  ...
  const std::vector<double> update_select_delete_ratio = {0.8, 0.2, 0.0};
  // NOLINTNEXTLINE
  for (auto _ : state) {
    // Blow away log file after every benchmark iteration
    unlink(LOG_FILE_NAME);
