...
BENCHMARK_DEFINE_F(TPCCBenchmark, ScaleFactor4WithGCMetrics)(benchmark::State &state) {
  ...
  auto curr_num_precomputed_txns = num_precomputed_txns_per_worker_ / ((terrier::BenchmarkConfig::num_threads + 2  - 1) / 2);

  if (terrier::BenchmarkConfig::num_threads > 20)
    curr_num_precomputed_txns *= 2;
  std::cout << "cooperative DAF | num daf thread:" << terrier::BenchmarkConfig::num_daf_threads << " num worker threads:" << terrier::BenchmarkConfig::num_threads << " txns per thread:" << curr_num_precomputed_txns << std::endl;
  // Precompute all of the input arguments for every txn to be run. We want to avoid the overhead at benchmark time
  const auto precomputed_args = PrecomputeArgs(&generator_, txn_weights_, terrier::BenchmarkConfig::num_threads,
  ...
  for (auto _ : state) {
    ...
    bool shut_down = false;
    {
      common::ScopedTimer<std::chrono::milliseconds> timer(&elapsed_ms);
      for (uint32_t i = 0; i < terrier::BenchmarkConfig::num_threads; i++) {
        thread_pool.SubmitTask([i, tpcc_db, &txn_manager, &precomputed_args, &deferred_action_manager, &workers, metrics_manager, &num_actual_processed, &shut_down] {
          metrics_manager->RegisterThread();
          Workload(i,
                   tpcc_db,
                   &txn_manager,
