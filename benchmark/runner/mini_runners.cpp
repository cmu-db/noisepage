...
#define TIGHT_LOOP_OPERATION(type, name, op)  \
  ...
  type __##type##_##name (size_t count) {     \
    type iterator = 1;                        \
...
class MiniRunners : public benchmark::Fixture {
 public:
  ...
  void BenchmarkArithmetic(brain::ExecutionOperatingUnitType type, size_t num_elem) {
    ...
    auto exec_ctx = std::make_unique<execution::exec::ExecutionContext>(GetDatabaseOid(), common::ManagedPointer(txn), nullptr, nullptr, common::ManagedPointer(accessor));
...
// NOLINTNEXTLINE
BENCHMARK_DEFINE_F(MiniRunners, Runner)(benchmark::State &state) {
  for (auto _ : state) {
    uint64_t elapsed_ms;
    {
      ...
      BenchmarkArithmetic(static_cast<brain::ExecutionOperatingUnitType>(state.range(0)), static_cast<size_t>(state.range(1)));
    }
  }
...
BENCHMARK_REGISTER_F(MiniRunners, Runner)
