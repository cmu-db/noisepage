...
class Deck {
 public:
  ...
  Deck(uint32_t w_new_order, uint32_t w_payment, uint32_t w_order_status, uint32_t w_delivery, uint32_t w_stock_level) {
    ...
    uint32_t min_new_order = static_cast<double>(std::ceil(w_new_order / 100.0 * 23));
    uint32_t min_payment = static_cast<double>(std::ceil(w_payment / 100.0 * 23));
    uint32_t min_order_status = static_cast<double>(std::ceil(w_order_status / 100.0 * 23));
    uint32_t min_delivery = static_cast<double>(std::ceil(w_delivery / 100.0 * 23));
    uint32_t min_stock_level = static_cast<double>(std::ceil(w_stock_level / 100.0 * 23));
