...
SequenceTuning::SequenceTuning(common::ManagedPointer<Pilot> pilot,
  ...
  for (auto &action : candidate_actions)
    if (structure_map_.at(action)->GetSQLCommand().rfind("create index", 0) == 0) {
      ...
      SELFDRIVING_LOG_DEBUG("Candidate structure: ID {} Command {}", action, structure_map_.at(action)->GetSQLCommand());
    }
...
void SequenceTuning::BestAction(
    std::vector<std::set<std::pair<const std::string, catalog::db_oid_t>>> *best_actions_seq, uint64_t memory_constraint) {
  ...
  for (auto structure_id : candidate_structures_) {
    ...
    std::vector<std::vector<std::set<action_id_t>>>
        singleton_action_repeated (end_segment_index_ + 1, singleton_action);

    double best_path_cost =
        GraphSolver(pilot_, forecast_, end_segment_index_, structure_map_, default_segment_cost_, singleton_action_repeated, 0.0)
    ...
    best_path_for_structure.emplace(structure_id, std::make_tuple(best_path_for_curr_structure, best_config_set_for_curr_action,
                                                                  best_path_cost));
  }
...
double SequenceTuning::UnionPair(const std::vector<std::set<action_id_t>> &seq_one,
  ...
  double best_unioned_dist =
      GraphSolver(pilot_, forecast_, end_segment_index_, structure_map_, default_segment_cost_, candidate_structures_by_segment, 0.0)
          .RecoverShortestPath(merged_solution, merged_config_set);
  return best_unioned_dist;
}

void SequenceTuning::GreedySeq(
    const std::map<action_id_t, std::tuple<std::vector<std::set<action_id_t>>,
                                           std::set<std::set<action_id_t>>,
                                           double>> &best_path_for_structure,
    std::vector<std::set<action_id_t>> *best_final_config_path) {

  // initialize C to contain all (binary) configs for each potential structure
  std::set<std::set<action_id_t>> C;
  // initialize P as paths found in cost-based pruning
  std::set<std::pair<double, uint64_t>> P;
  std::vector<std::tuple<std::vector<std::set<action_id_t>>, std::set<std::set<action_id_t>>,
                         double>> all_paths;

  auto ct = 0;
  for (auto const& path_it : best_path_for_structure) {
    auto config_set = std::get<1>(path_it.second);
    C.insert(config_set.begin(), config_set.end());
    all_paths.push_back(path_it.second);
    P.emplace(std::get<2>(path_it.second), ct);
    ct++;
  }

  MergeConfigs(&P, &all_paths, &C);

  std::vector<std::set<action_id_t>> candidate_structures;
  candidate_structures.insert(candidate_structures.begin(), C.begin(), C.end());

  std::vector<std::vector<std::set<action_id_t>>>
      candidate_structures_by_segment (end_segment_index_ + 1, candidate_structures);

  double final_soln_cost =
      GraphSolver(pilot_, forecast_, end_segment_index_, structure_map_, default_segment_cost_, candidate_structures_by_segment, 0.0)
...
void SequenceTuning::ExtractActionsFromConfigPath(
  ...
  for (auto config_idx = 1; config_idx < best_final_config_path.size(); config_idx ++) {
    ...
    for (auto structure_id : best_final_config_path.at(config_idx))
      if (best_final_config_path.at(config_idx - 1).find(structure_id) == best_final_config_path.at(config_idx - 1).end())
...
void SequenceTuning::MergeConfigs(std::set<std::pair<double, uint64_t>> *P,
                                  std::vector<std::tuple<std::vector<std::set<action_id_t>>,
                                                         std::set<std::set<action_id_t>>,
                                                         double>> *all_paths,
                                  std::set<std::set<action_id_t>> *C) {
  bool step_3_finished = false;
  while (!P->empty() && !step_3_finished) {
    uint64_t least_cost_index = P->begin()->second;
    double least_cost = P->begin()->first;
    auto best_couple_pair = *(P->begin());
    ...
    for (auto path_it : *P) {
      ...
      double union_cost =
          UnionPair(std::get<0>(all_paths->at(least_cost_index)), std::get<0>(all_paths->at(second_index)),
                    &merged_solution, &merged_config_set);
    ...
    if (best_union_cost < least_cost) {
      auto s = P->size();
      P->erase(best_couple_pair);
      NOISEPAGE_ASSERT(P->size() + 1 == s, "Expect P->erase decrease size by 1");
      all_paths->emplace_back(best_merged_solution, best_merged_config_set, best_union_cost);
      P->emplace(best_union_cost, all_paths->size() - 1);
      SELFDRIVING_LOG_DEBUG("[GreedySeq] new path added to P with distance {} number of config {}", best_union_cost,
                           best_merged_config_set.size());
      NOISEPAGE_ASSERT(P->size() == s, "Expect new unioned path doesn't exist in P");
    } else {
