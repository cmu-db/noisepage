...
void MonteCarloTreeSearch::BestAction(
    uint64_t simulation_number,
  ...
  while (!curr_node->IsLeaf()) {
    ...
    best_actions_seq->push_back({std::make_pair(action_map_.at(best_child->GetCurrentAction())->GetSQLCommand(),
