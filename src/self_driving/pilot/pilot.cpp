...
Pilot::Pilot(
    ...
    : catalog_(catalog), metrics_thread_(metrics_thread), ms_manager_(model_server_manager),
      settings_manager_(settings_manager), stats_storage_(stats_storage), txn_manager_(txn_manager),
...
void Pilot::ExecuteForecast() {
  ...
  auto pipeline_data = PilotUtil::CollectPipelineFeatures(common::ManagedPointer<selfdriving::Pilot>(this), common::ManagedPointer(forecast_));
