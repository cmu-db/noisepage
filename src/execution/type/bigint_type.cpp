...
// bigint_type.cpp
//
// Identification: src/execution/type/bigint_type.cpp
//
...
struct CompareBigInt : public TypeSystem::SimpleComparisonHandleNull {
  ...
  Value CompareForSortImpl(CodeGen &codegen, const Value &left, const Value &right) const override {
    ...
    return Value{Integer::Instance(), codegen->CreateTrunc(diff, codegen.Int32Type()), nullptr, nullptr};
  }
};
...
struct Abs : public TypeSystem::UnaryOperatorHandleNull {
  bool SupportsType(const Type &type) const override { return type.GetSqlType() == BigInt::Instance(); }
  ...
  Value Impl(CodeGen &codegen, const Value &val, const TypeSystem::InvocationContext &ctx) const override {
    ...
    Sub sub;
    // Zero place-holder
    auto zero = Value{type::BigInt::Instance(), codegen.Const64(0)};
...
struct Add : public TypeSystem::BinaryOperatorHandleNull {
  bool SupportsTypes(const Type &left_type, const Type &right_type) const override {
    return left_type.GetSqlType() == BigInt::Instance() && left_type == right_type;
  }
...
void BigInt::GetTypeForMaterialization(CodeGen &codegen, llvm::Type *&val_type, llvm::Type *&len_type) const {
  val_type = codegen.Int64Type();
  len_type = nullptr;
}

llvm::Function *BigInt::GetInputFunction(CodeGen &codegen, UNUSED_ATTRIBUTE const Type &type) const {
  return NumericFunctionsProxy::InputBigInt.GetFunction(codegen);
}

llvm::Function *BigInt::GetOutputFunction(CodeGen &codegen, UNUSED_ATTRIBUTE const Type &type) const {
