...
// varbinary_type.cpp
//
// Identification: src/execution/type/varbinary_type.cpp
//
...
struct CompareVarbinary : public TypeSystem::ExpensiveComparisonHandleNull {
  bool SupportsTypes(const type::Type &left_type, const type::Type &right_type) const override {
    return left_type.GetSqlType() == Varbinary::Instance() && left_type == right_type;
  }
...
std::vector<TypeSystem::ComparisonInfo> kComparisonTable = {{kCompareVarbinary}};
...
void Varbinary::GetTypeForMaterialization(CodeGen &codegen, llvm::Type *&val_type, llvm::Type *&len_type) const {
  val_type = codegen.CharPtrType();
  len_type = codegen.Int32Type();
}

llvm::Function *Varbinary::GetInputFunction(UNUSED_ATTRIBUTE CodeGen &codegen,
                                            UNUSED_ATTRIBUTE const Type &type) const {
  throw NotImplementedException{"Blob input not implemented yet"};
}

llvm::Function *Varbinary::GetOutputFunction(CodeGen &codegen, UNUSED_ATTRIBUTE const Type &type) const {
