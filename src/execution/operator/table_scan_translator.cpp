...
//
// Identification: src/execution/operator/table_scan_translator.cpp
//
// Copyright (c) 2015-2018, Carnegie Mellon University Database Group
//
//===----------------------------------------------------------------------===//

#include "execution/operator/table_scan_translator.h"
...
class TableScanTranslator::AttributeAccess : public RowBatch::AttributeAccess {
 public:
  AttributeAccess(const TileGroup::TileGroupAccess &access, const planner::AttributeInfo *ai)
 ...
 */
class TableScanTranslator::ScanConsumer : public ScanCallback {
 public:
  // Constructor
  ScanConsumer(ConsumerContext &ctx, const planner::SeqScanPlan &plan, Vector &selection_vector)
      : ctx_(ctx),
        plan_(plan),
  ...
  void TileGroupStart(CodeGen &, llvm::Value *tile_group_id, llvm::Value *tile_group_ptr) override {
    tile_group_id_ = tile_group_id;
    tile_group_ptr_ = tile_group_ptr;
  }

  // The code that forms the body of the scan loop
  void ProcessTuples(CodeGen &codegen, llvm::Value *tid_start, llvm::Value *tid_end,
 ...
 private:
  void SetupRowBatch(RowBatch &batch, TileGroup::TileGroupAccess &tile_group_access,
                     std::vector<AttributeAccess> &access) const;

  void FilterRowsByVisibility(CodeGen &codegen, llvm::Value *tid_start, llvm::Value *tid_end,
  ...
  void FilterRowsByPredicate(CodeGen &codegen, const TileGroup::TileGroupAccess &access, llvm::Value *tid_start,
...
TableScanTranslator::TableScanTranslator(const planner::SeqScanPlan &scan, CompilationContext &context,
                                         Pipeline &pipeline)
    : OperatorTranslator(scan, context, pipeline), table_(*scan.GetTable()) {
  // Set ourselves as the source of the pipeline
  auto parallelism = scan.IsParallel() ? Pipeline::Parallelism::Parallel : Pipeline::Parallelism::Serial;
...
llvm::Value *TableScanTranslator::LoadTablePtr(CodeGen &codegen) const {
  ...
  return codegen.Call(StorageManagerProxy::GetTableWithOid, {GetStorageManagerPtr(), db_oid, table_oid});
}
...
void TableScanTranslator::ProduceSerial() const {
  auto producer = [this](ConsumerContext &ctx) {
    ...
    auto predicate = const_cast<expression::AbstractExpression *>(GetScanPlan().GetPredicate());
    ...
    table_.GenerateScan(codegen, table_ptr, nullptr, nullptr, vec_size, predicate_ptr, num_preds, scan_consumer);
  };
...
void TableScanTranslator::ProduceParallel() const {
  ...
  auto *dispatcher = RuntimeFunctionsProxy::ExecuteTableScan.GetFunction(codegen);
  std::vector<llvm::Value *> dispatch_args = {codegen.Const32(table.GetDatabaseOid()), codegen.Const32(table.GetOid())};

  // Our function needs to know the start and stop tile groups to scan
  std::vector<llvm::Type *> pipeline_arg_types = {codegen.Int64Type(), codegen.Int64Type()};
  ...
  auto producer = [this, &codegen](ConsumerContext &ctx, const std::vector<llvm::Value *> params) {
    ...
    ScanConsumer scan_consumer{ctx, GetScanPlan(), position_list};
    table_.GenerateScan(codegen, table_ptr, tilegroup_start, tilegroup_end, vec_size, predicate_ptr, num_preds,
                        scan_consumer);
  };
...
const planner::SeqScanPlan &TableScanTranslator::GetScanPlan() const { return GetPlanAs<planner::SeqScanPlan>(); }
...
void TableScanTranslator::ScanConsumer::ProcessTuples(CodeGen &codegen, llvm::Value *tid_start, llvm::Value *tid_end,
  ...
  if (predicate != nullptr) {
    ...
    FilterRowsByPredicate(codegen, tile_group_access, tid_start, tid_end, selection_vector_);
  }
  ...
  RowBatch batch{ctx_.GetCompilationContext(), tile_group_id_, tid_start, tid_end, selection_vector_, true};
...
void TableScanTranslator::ScanConsumer::SetupRowBatch(RowBatch &batch, TileGroup::TileGroupAccess &tile_group_access,
  ...
  for (oid_t col_idx = 0; col_idx < output_col_ids.size(); col_idx++) {
    ...
    LOG_TRACE("Adding attribute '%s.%s' (%p) into row batch", plan_.GetTable()->GetName().c_str(),
              attribute->name.c_str(), attribute);
    batch.AddAttribute(attribute, &access[col_idx]);
  }
}

void TableScanTranslator::ScanConsumer::FilterRowsByVisibility(CodeGen &codegen, llvm::Value *tid_start,
                                                               llvm::Value *tid_end, Vector &selection_vector) const {
  ...
  llvm::Value *out_idx = codegen.Call(TransactionRuntimeProxy::PerformVisibilityCheck,
                                      {txn, tile_group_ptr_, tid_start, tid_end, raw_sel_vec});
  selection_vector.SetNumElements(out_idx);
}

void TableScanTranslator::ScanConsumer::FilterRowsByPredicate(CodeGen &codegen,
                                                              const TileGroup::TileGroupAccess &access,
                                                              llvm::Value *tid_start, llvm::Value *tid_end,
                                                              Vector &selection_vector) const {
  // The batch we're filtering
  RowBatch batch{ctx_.GetCompilationContext(), tile_group_id_, tid_start, tid_end, selection_vector, true};
...
void TableScanTranslator::ScanConsumer::PerformReads(CodeGen &codegen, Vector &selection_vector) const {
  ExecutionConsumer &ec = ctx_.GetCompilationContext().GetExecutionConsumer();
  llvm::Value *txn = ec.GetTransactionPtr(ctx_.GetCompilationContext());
  llvm::Value *raw_sel_vec = selection_vector.GetVectorPtr();
  ...
  llvm::Value *out_idx = codegen.Call(TransactionRuntimeProxy::PerformVectorizedRead,
