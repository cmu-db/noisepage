...
//
// Identification: src/execution/operator/global_group_by_translator.cpp
//
...
GlobalGroupByTranslator::GlobalGroupByTranslator(const planner::AggregatePlan &plan, CompilationContext &context,
  ...
  TERRIER_ASSERT(aggregate_storage->isStructTy(), "Must be a struct type.");
...
void GlobalGroupByTranslator::Produce() const {
  ...
  auto producer = [this](ConsumerContext &ctx) {
    ...
    TERRIER_ASSERT(agg_terms.size() == aggregate_vals.size(), "Number of terms and sizes must be the same.");
