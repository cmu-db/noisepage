...
//
// Identification: src/execution/operator/hash_group_by_translator.cpp
//
// Copyright (c) 2015-2018, Carnegie Mellon University Database Group
//
//===----------------------------------------------------------------------===//

#include "execution/operator/hash_group_by_translator.h"
...
// Constructor
HashGroupByTranslator::HashGroupByTranslator(const planner::AggregatePlan &group_by, CompilationContext &context,
                                             Pipeline &pipeline)
  ...
  hash_table_id_ = query_state.RegisterState("groupBy", OAHashTableProxy::GetType(codegen));
  ...
  hash_table_ = OAHashTable{codegen, key_type, aggregation_.GetAggregatesStorageSize()};
}
...
void HashGroupByTranslator::Produce() const {
  ...
  auto producer = [this](ConsumerContext &ctx) {
    ...
    hash_table_.VectorizedIterate(codegen, LoadStatePtr(hash_table_id_), selection_vec, produce_results);
  };

  GetPipeline().RunSerial(producer);
}

void HashGroupByTranslator::Consume(ConsumerContext &context, RowBatch &batch) const {
  if (!UsePrefetching()) {
  ...
  auto *raw_vec = codegen.AllocateBuffer(codegen.Int64Type(), OAHashTable::kDefaultGroupPrefetchSize, "pfVector");
  Vector hashes{raw_vec, OAHashTable::kDefaultGroupPrefetchSize, codegen.Int64Type()};

  auto group_prefetch = [&](RowBatch::VectorizedIterateCallback::IterationInstance &iter_instance) {
    llvm::Value *p = codegen.Const32(0);
    llvm::Value *end = codegen->CreateSub(iter_instance.end, iter_instance.start);
    ...
    lang::Loop prefetch_loop{codegen, codegen->CreateICmpULT(p, end), {{"p", p}}};
    {
      ...
      RowBatch::Row row = batch.GetRowAt(codegen->CreateAdd(p, iter_instance.start));
      ...
      hash_table_.PrefetchBucket(codegen, LoadStatePtr(hash_table_id_), hash_val, OAHashTable::PrefetchType::Read,
    ...
    p = codegen.Const32(0);
    std::vector<lang::Loop::LoopVariable> loop_vars = {{"p", p}, {"writeIdx", iter_instance.write_pos}};
    lang::Loop process_loop{codegen, codegen->CreateICmpULT(p, end), loop_vars};
    {
      ...
      Value row_hash{type::Integer::Instance(), hashes.GetValue(codegen, p)};
      ...
      // End prefetch loop
      p = codegen->CreateAdd(p, codegen.Const32(1));
      process_loop.LoopEnd(codegen->CreateICmpULT(p, end), {p, tracker.GetFinalOutputPos()});
    }
  ...
  batch.VectorizedIterate(codegen, OAHashTable::kDefaultGroupPrefetchSize, group_prefetch);
}

// Consume the tuples from the context, grouping them into the hash table
void HashGroupByTranslator::Consume(ConsumerContext &, RowBatch::Row &row) const {
  CodeGen &codegen = GetCodeGen();
  ...
  if (row.HasAttribute(&OAHashTable::kHashAI)) {
    Value hash_val = row.DeriveValue(codegen, &OAHashTable::kHashAI);
...
void HashGroupByTranslator::CollectHashKeys(RowBatch::Row &row, std::vector<Value> &key) const {
  CodeGen &codegen = GetCodeGen();
  const auto &plan = GetPlanAs<planner::AggregatePlan>();
...
HashGroupByTranslator::AggregateFinalizer::AggregateFinalizer(const Aggregation &aggregation,
...
HashGroupByTranslator::AggregateAccess::AggregateAccess(AggregateFinalizer &finalizer, uint32_t agg_index)
    : finalizer_(finalizer), agg_index_(agg_index) {}

Value HashGroupByTranslator::AggregateAccess::Access(CodeGen &codegen, RowBatch::Row &row) {
...
HashGroupByTranslator::ProduceResults::ProduceResults(ConsumerContext &ctx, const planner::AggregatePlan &plan,
                                                      const Aggregation &aggregation)
    : ctx_(ctx), plan_(plan), aggregation_(aggregation) {}

void HashGroupByTranslator::ProduceResults::ProcessEntries(CodeGen &codegen, llvm::Value *start, llvm::Value *end,
                                                           Vector &selection_vector,
                                                           HashTable::HashTableAccess &access) const {
  RowBatch batch{ctx_.GetCompilationContext(), start, end, selection_vector, true};
  ...
  for (uint64_t i = 0; i < grouping_ais.size(); i++) {
    LOG_DEBUG("Adding aggregate key attribute '%s' (%p) to batch", grouping_ais[i]->name.c_str(), grouping_ais[i]);
    batch.AddAttribute(grouping_ais[i], &accessors[i]);
  }
  for (uint64_t i = 0; i < aggregates.size(); i++) {
    auto &agg_term = aggregates[i];
    LOG_DEBUG("Adding aggregate attribute '%s' (%p) to batch", agg_term.agg_ai.name.c_str(), &agg_term.agg_ai);
  ...
  if (project_info != nullptr) {
    ProjectionTranslator::AddNonTrivialAttributes(batch, *project_info, derived_attribute_accessors);
  }
...
// Constructor
HashGroupByTranslator::ConsumerProbe::ConsumerProbe(CompilationContext &context, const Aggregation &aggregation,
                                                    const std::vector<Value> &next_vals,
...
void HashGroupByTranslator::ConsumerProbe::ProcessEntry(CodeGen &codegen, llvm::Value *data_area) const {
...
HashGroupByTranslator::ConsumerInsert::ConsumerInsert(const Aggregation &aggregation,
                                                      const std::vector<Value> &initial_vals,
                                                      const std::vector<Value> &grouping_keys)
    : aggregation_(aggregation), initial_vals_(initial_vals), grouping_keys_(grouping_keys) {}

// Given free storage space in the hash table, store the initial values of all
// the aggregates
void HashGroupByTranslator::ConsumerInsert::StoreValue(CodeGen &codegen, llvm::Value *space) const {
  aggregation_.CreateInitialValues(codegen, space, initial_vals_, grouping_keys_);
}

llvm::Value *HashGroupByTranslator::ConsumerInsert::GetValueSize(CodeGen &codegen) const {
