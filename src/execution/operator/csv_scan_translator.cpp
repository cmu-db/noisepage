...
//
// Identification: src/execution/operator/csv_scan_translator.cpp
//
// Copyright (c) 2015-2018, Carnegie Mellon University Database Group
//
//===----------------------------------------------------------------------===//

#include "execution/operator/csv_scan_translator.h"
...
CSVScanTranslator::CSVScanTranslator(const planner::CSVScanPlan &scan, CompilationContext &context, Pipeline &pipeline)
    : OperatorTranslator(scan, context, pipeline) {
  // Register the CSV scanner instance
  auto &query_state = context.GetQueryState();
  scanner_id_ = query_state.RegisterState("csvScanner", CSVScannerProxy::GetType(GetCodeGen()));
...
void CSVScanTranslator::InitializeQueryState() {
  ...
  std::vector<type::Type> col_types_vec;
  col_types_vec.reserve(num_cols);
  for (const auto *ai : output_attributes_) {
    col_types_vec.push_back(ai->type);
  }
  llvm::Value *raw_col_type_bytes =
      codegen.ConstGenericBytes(col_types_vec.data(), static_cast<uint32_t>(col_types_vec.capacity()), "colTypes");
  llvm::Value *output_col_types =
      codegen->CreatePointerCast(raw_col_type_bytes, TypeProxy::GetType(codegen)->getPointerTo());

  // Now create a pointer to the consumer function
  using ConsumerFuncType = void (*)(void *);
  llvm::Value *consumer_func =
      codegen->CreatePointerCast(consumer_func_, proxy::TypeBuilder<ConsumerFuncType>::GetType(codegen));
  ...
  llvm::Value *query_state_ptr = codegen->CreatePointerCast(codegen.GetState(), codegen.VoidPtrType());

  // Call CSVScanner::Init()
  codegen.Call(CSVScannerProxy::Init,
               {scanner_ptr, exec_ctx_ptr, file_path, output_col_types, codegen.Const32(num_cols), consumer_func,
                query_state_ptr, codegen.Const8(scan.GetDelimiterChar()), codegen.Const8(scan.GetQuoteChar()),
...
class CSVColumnAccess : public RowBatch::AttributeAccess {
 public:
  CSVColumnAccess(const planner::AttributeInfo *ai, llvm::Value *csv_columns, std::string null_str,
                  llvm::Value *runtime_null_str)
      : ai_(ai), csv_columns_(csv_columns), null_str_(std::move(null_str)), runtime_null_(runtime_null_str) {}
   ...
   * otherwise.
   */
  llvm::Value *IsNull(CodeGen &codegen, llvm::Value *data_ptr, llvm::Value *data_len) const {
    uint32_t null_str_len = static_cast<uint32_t>(null_str_.length());

    // Is the length of the column value the same as the NULL string?
    llvm::Value *eq_len = codegen->CreateICmpEQ(data_len, codegen.Const32(null_str_len));
    ...
    lang::If check_null{codegen, eq_len};
    {
      ...
      cmp_res = codegen.Memcmp(data_ptr, runtime_null_, codegen.Const64(null_str_.length()));
   ...
   */
  Value LoadValueIgnoreNull(CodeGen &codegen, llvm::Value *type, llvm::Value *data_ptr, llvm::Value *data_len) const {
    ...
    if (SqlType().IsVariableLength()) {
      ...
      return Value{ai_->type, str_ptr, str_len, codegen.ConstBool(false)};
    } else {
      return Value{ai_->type, raw_val, nullptr, codegen.ConstBool(false)};
    }
  }
  ...
  Value Access(CodeGen &codegen, UNUSED_ATTRIBUTE RowBatch::Row &row) override {
    ...
    auto *type =
        codegen->CreateConstInBoundsGEP2_32(CSVScannerColumnProxy::GetType(codegen), Columns(), ColumnIndex(), 0);
    auto *data_ptr = codegen->CreateLoad(
        codegen->CreateConstInBoundsGEP2_32(CSVScannerColumnProxy::GetType(codegen), Columns(), ColumnIndex(), 1));
    auto *data_len = codegen->CreateLoad(
        codegen->CreateConstInBoundsGEP2_32(CSVScannerColumnProxy::GetType(codegen), Columns(), ColumnIndex(), 2));
    ...
    Value valid_val, null_val;
    lang::If is_null{codegen, codegen->CreateNot(IsNull(codegen, data_ptr, data_len))};
    {
      // Load valid
...
void CSVScanTranslator::DefineAuxiliaryFunctions() {
  ...
  FunctionDeclaration decl{codegen.GetCodeContext(), "consumer", FunctionDeclaration::Visibility::Internal,
  ...
  FunctionBuilder scan_consumer{codegen.GetCodeContext(), decl};
  {
    ...
    RowBatch one{GetCompilationContext(), codegen.Const32(0), codegen.Const32(1), v, false};

    // Load the pointer to the columns view
    llvm::Value *cols = codegen->CreateLoad(
        codegen->CreateConstInBoundsGEP2_32(CSVScannerProxy::GetType(codegen), LoadStatePtr(scanner_id_), 0, 1));
    ...
    for (uint32_t i = 0; i < output_attributes_.size(); i++) {
      column_accessors.emplace_back(output_attributes_[i], cols, scan.GetNullString(), null_str);
    }
    for (uint32_t i = 0; i < output_attributes_.size(); i++) {
