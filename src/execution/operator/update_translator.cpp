...
//
// Identification: src/execution/operator/update_translator.cpp
//
// Copyright (c) 2015-17, Carnegie Mellon University Database Group
//
//===----------------------------------------------------------------------===//

#include "execution/operator/update_translator.h"
...
UpdateTranslator::UpdateTranslator(const planner::UpdatePlan &update_plan, CompilationContext &context,
                                   Pipeline &pipeline)
    : OperatorTranslator(update_plan, context, pipeline), table_storage_(*update_plan.GetTable()->GetSchema()) {
  ...
  // Prepare for updater
  updater_state_id_ = context.GetQueryState().RegisterState("updater", UpdaterProxy::GetType(GetCodeGen()));
}
...
void UpdateTranslator::InitializeQueryState() {
  ...
  llvm::Value *target_vector_ptr = codegen->CreateIntToPtr(
      codegen.Const64((int64_t)project_info->GetTargetList().data()), TargetProxy::GetType(codegen)->getPointerTo());
  llvm::Value *target_vector_size_ptr = codegen.Const32((int32_t)project_info->GetTargetList().size());
...
void UpdateTranslator::Consume(ConsumerContext &, RowBatch::Row &row) const {
  ...
  uint32_t column_num = static_cast<uint32_t>(target_list.size() + direct_map_list.size());
  ...
  std::vector<llvm::Value *> prep_args = {updater, row.GetTileGroupID(), row.GetTID(codegen)};
  ...
  llvm::Value *prepare_cond =
      codegen->CreateICmpNE(codegen->CreatePtrToInt(tuple_ptr, codegen.Int64Type()), codegen.Const64(0));
