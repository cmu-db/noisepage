...
//
// Identification: src/execution/bloom_filter_accessor.cpp
//
...
void BloomFilterAccessor::Init(CodeGen &codegen, llvm::Value *bloom_filter, uint64_t estimated_num_tuples) const {
  codegen.Call(BloomFilterProxy::Init, {bloom_filter, codegen.Const64(estimated_num_tuples)});
}
...
void BloomFilterAccessor::Add(CodeGen &codegen, llvm::Value *bloom_filter,
  ...
  llvm::Value *seed_hash1 = Hash::HashValues(codegen, key, util::BloomFilter::kSeedHashFuncs[0]);
  llvm::Value *seed_hash2 = Hash::HashValues(codegen, key, util::BloomFilter::kSeedHashFuncs[1]);
  ...
  lang::Loop add_loop{codegen, end_cond, {{"i", index}}};
  {
    ...
    llvm::Value *new_byte = codegen->CreateOr(existing_byte, codegen->CreateShl(codegen.Const8(1), bit_offset_in_byte));
...
llvm::Value *BloomFilterAccessor::Contains(CodeGen &codegen, llvm::Value *bloom_filter,
                                           const std::vector<Value> &key) const {
  // Index of current hash being calculated
  llvm::Value *index = codegen.Const64(0);
  llvm::Value *num_hashes = LoadBloomFilterField(codegen, bloom_filter, 0);
  ...
  StoreBloomFilterField(codegen, bloom_filter, 4, codegen->CreateAdd(num_probe, codegen.Const64(1)));
  ...
  lang::Loop add_loop{codegen, end_cond, {{"i", index}}};
  {
    ...
    llvm::Value *existing_byte = codegen->CreateLoad(byte_ptr);
    llvm::Value *and_val = codegen->CreateAnd(existing_byte, codegen->CreateShl(codegen.Const8(1), bit_offset_in_byte));
    ...
    lang::If bit_not_set{codegen, equal_zero, "BitNotSet"};
    {
      ...
      StoreBloomFilterField(codegen, bloom_filter, 3, codegen->CreateAdd(num_misses, codegen.Const64(1)));
      add_loop.Break();
    }
    bit_not_set.EndIf();
  ...
  llvm::Value *contains = codegen->CreateSelect(loop_to_end, codegen.ConstBool(true), codegen.ConstBool(false));

  return contains;
}

llvm::Value *BloomFilterAccessor::CalculateHash(CodeGen &codegen, llvm::Value *index, llvm::Value *seed_hash1,
  ...
  llvm::Value *hash =
      codegen->CreateSelect(is_first, seed_hash1, codegen->CreateSelect(is_second, seed_hash2, combined_hash));
  return hash;
}

// Given the hash, find the corresponding byte that contains the bit
void BloomFilterAccessor::LocateBit(CodeGen &codegen, llvm::Value *bloom_filter, llvm::Value *hash,
                                    llvm::Value *&bit_offset_in_byte, llvm::Value *&byte_ptr) const {
  ...
  byte_ptr = codegen->CreateInBoundsGEP(codegen.ByteType(), byte_array, byte_offset);
}

void BloomFilterAccessor::StoreBloomFilterField(CodeGen &codegen, llvm::Value *bloom_filter, uint32_t field_id,
  ...
  codegen->CreateStore(new_field_val,
                       codegen->CreateConstInBoundsGEP2_32(bloom_filter_type, bloom_filter, 0, field_id));
}

llvm::Value *BloomFilterAccessor::LoadBloomFilterField(CodeGen &codegen, llvm::Value *bloom_filter,
                                                       uint32_t field_id) const {
  llvm::Type *bloom_filter_type = BloomFilterProxy::GetType(codegen);
  return codegen->CreateLoad(codegen->CreateConstInBoundsGEP2_32(bloom_filter_type, bloom_filter, 0, field_id));
}
