...
void StandardTemplatedCastOperation(const Vector &source, Vector *target) {
  UnaryOperationExecutor::Execute<InType, OutType, execution::sql::Cast<InType, OutType>, IgnoreNull>(source, target);
}
...
void CastToStringOperation(const Vector &source, Vector *target) {
  ...
  execution::sql::Cast<InType, std::string> cast_op;
...
void CastNumericOperation(const Vector &source, Vector *target, SqlTypeId target_type) {
  switch (target_type) {
    ...
    case SqlTypeId::Varchar:
      ...
      break;
    default:
      throw NOT_IMPLEMENTED_EXCEPTION("Unsupported cast.");
  }
}
...
void VectorOps::Cast(const Vector &source, Vector *target, SqlTypeId source_type, SqlTypeId target_type) {
  switch (source_type) {
    case SqlTypeId::Boolean:
