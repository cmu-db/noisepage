...
BloomFilter::BloomFilter() noexcept
    : memory_(nullptr), blocks_(nullptr), block_mask_(0), num_additions_(0), lazily_added_hashes_(nullptr) {}

BloomFilter::BloomFilter(MemoryPool *memory)
    : memory_(memory), blocks_(nullptr), block_mask_(0), num_additions_(0), lazily_added_hashes_(nullptr) {}
...
void BloomFilter::Init(MemoryPool *memory, uint32_t expected_num_elems) {
  ...
  uint64_t num_bits = common::MathUtil::PowerOf2Ceil(kBitsPerElement * expected_num_elems);
