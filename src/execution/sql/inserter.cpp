...
terrier::storage::ProjectedRow *terrier::execution::sql::Inserter::GetIndexPR(terrier::catalog::index_oid_t index_oid) {
  index_ = exec_ctx_->GetAccessor()->GetIndex(index_oid);
  terrier::storage::ProjectedRowInitializer pri = index_->GetProjectedRowInitializer();
  auto txn = exec_ctx_->GetTxn();
...
bool terrier::execution::sql::Inserter::IndexInsert() {
  return index_->Insert(exec_ctx_->GetTxn(), *index_pr_, table_tuple_slot_);
}
