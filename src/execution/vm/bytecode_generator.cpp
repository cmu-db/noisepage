...
void BytecodeGenerator::VisitSqlConversionCall(ast::CallExpr *call, ast::Builtin builtin) {
  ...
  switch (builtin) {
    ...
    case ast::Builtin::DecimalSetPrecision: {
      ...
      GetEmitter()->Emit(Bytecode::DecimalSetPrecision, dest, fixed_decimal, precision);
      break;
    }
    case ast::Builtin::DecimalRescalePrecision: {
      auto fixed_decimal = VisitExpressionForRValue(call->Arguments()[0]);
      auto precision = VisitExpressionForRValue(call->Arguments()[1]);
      GetEmitter()->Emit(Bytecode::DecimalRescalePrecision, dest, fixed_decimal, precision);
      break;
    }
...
void BytecodeGenerator::VisitBuiltinPRCall(ast::CallExpr *call, ast::Builtin builtin) {
  ...
  switch (builtin) {
    ...
    case ast::Builtin::PRGetDecimal: {
      LocalVar val =
          GetExecutionResult()->GetOrCreateDestination(ast::BuiltinType::Get(ctx, ast::BuiltinType::Decimal));
      auto col_idx = static_cast<uint16_t>(call->Arguments()[1]->As<ast::LitExpr>()->Int64Val());
