...
void PrettyPrintFuncInfo(std::ostream *os, const FunctionInfo &func) {
  ...
  *os << "  Frame size " << func.frame_size() << " bytes (" << func.NumParams() << " parameter"
  ...
  for (const auto &local : func.locals()) {
    ...
    *os << std::setw(static_cast<int>(max_local_len)) << std::right << local.Name() << ":  offset=" << std::setw(7)
        ...
        << " align=" << std::setw(7) << std::left << local.GetType()->Alignment() << " type=" << std::setw(7) << std::left
        << ast::Type::ToString(local.GetType()) << std::endl;
  }
}
