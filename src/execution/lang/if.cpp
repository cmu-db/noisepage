...
// if.cpp
//
// Identification: src/execution/lang/if.cpp
//
...
void If::ElseBlock(const std::string &name) {
  ...
  auto *new_branch = llvm::BranchInst::Create(then_bb_, else_bb_, branch_->getCondition());
...
Value If::BuildPHI(const Value &v1, const Value &v2) {
  ...
  std::vector<std::pair<Value, llvm::BasicBlock *>> vals = {
      {v1, last_bb_in_then_}, {v2, last_bb_in_else_ != nullptr ? last_bb_in_else_ : branch_->getParent()}};
...
llvm::Value *If::BuildPHI(llvm::Value *v1, llvm::Value *v2) {
  ...
  phi->addIncoming(v2, last_bb_in_else_ != nullptr ? last_bb_in_else_ : branch_->getParent());
  return phi;
}
