...
// hash_table.cpp
//
// Identification: src/execution/hash_table.cpp
//
// Copyright (c) 2015-2018, Carnegie Mellon University Database Group
//
//===----------------------------------------------------------------------===//

#include "execution/hash_table.h"
...
HashTable::HashTable(CodeGen &codegen, const std::vector<type::Type> &key_type, uint32_t value_size)
...
void HashTable::ProbeOrInsert(CodeGen &codegen, llvm::Value *ht_ptr, llvm::Value *hash, const std::vector<Value> &key,
  ...
  // Compute hash value
  llvm::Value *hash_val = hash != nullptr ? hash : Hash::HashValues(codegen, key);
  ...
  llvm::Value *bucket = codegen->CreateLoad(codegen->CreateInBoundsGEP(directory, {bucket_idx}));

  // Iterate the bucket chain until we get a NULL entry
  llvm::Value *null = codegen.NullPtr(llvm::cast<llvm::PointerType>(bucket->getType()));
  ...
  lang::Loop chain_loop{codegen, end_condition, {{"iter", bucket}}};
  {
    ...
    lang::If hash_match{codegen, codegen->CreateICmpEQ(entry_hash, hash_val), "hashMatch"};
    {
      // The hashes match, what about the keys?
      llvm::Value *keys_ptr = codegen->CreateConstInBoundsGEP2_32(ht_entry_type, entry, 1, 0);

      // Pull out the keys
      std::vector<Value> hash_entry_keys;
      llvm::Value *values_area = key_storage_.LoadValues(codegen, keys_ptr, hash_entry_keys);
  ...
  llvm::Value *ptr = codegen.Call(HashTableProxy::Insert, {ht_ptr, hash_val, needed_bytes});
  ...
  codegen->GetInsertBlock()->getParent()->getBasicBlockList().push_back(cont_bb);
  codegen->SetInsertPoint(cont_bb);
}

HashTable::ProbeResult HashTable::ProbeOrInsert(UNUSED_ATTRIBUTE CodeGen &codegen, UNUSED_ATTRIBUTE llvm::Value *ht_ptr,
                                                UNUSED_ATTRIBUTE llvm::Value *hash,
                                                UNUSED_ATTRIBUTE const std::vector<Value> &key) const {
  throw NotImplementedException{"ProbeOrInsert returning probe result not support in HashTable"};
}

void HashTable::Insert(CodeGen &codegen, llvm::Value *ht_ptr, llvm::Value *hash, const std::vector<Value> &key,
  ...
  llvm::Value *ptr = codegen.Call(HashTableProxy::InsertLazy, {ht_ptr, hash_val});
...
void HashTable::ReserveLazy(CodeGen &codegen, llvm::Value *ht_ptr, llvm::Value *thread_states,
                            uint32_t ht_state_offset) const {
  codegen.Call(HashTableProxy::ReserveLazy, {ht_ptr, thread_states, codegen.Const32(ht_state_offset)});
}

void HashTable::MergeLazyUnfinished(CodeGen &codegen, llvm::Value *global_ht, llvm::Value *local_ht) const {
  codegen.Call(HashTableProxy::MergeLazyUnfinished, {global_ht, local_ht});
}

void HashTable::Iterate(CodeGen &codegen, llvm::Value *ht_ptr, IterateCallback &callback) const {
  ...
  lang::Loop bucket_loop{codegen, bucket_cond, {{"bucketNum", bucket_num}}};
  {
    ...
    llvm::Value *bucket = codegen->CreateLoad(codegen->CreateGEP(buckets_ptr, bucket_num));
    llvm::Value *null_bucket = codegen.NullPtr(llvm::cast<llvm::PointerType>(bucket->getType()));

    lang::Loop chain_loop{codegen, codegen->CreateICmpNE(bucket, null_bucket), {{"entry", bucket}}};
    {
      llvm::Type *ht_entry_type = EntryProxy::GetType(codegen);
      llvm::Value *entry = chain_loop.GetLoopVar(0);
      llvm::Value *entry_data = codegen->CreateConstInBoundsGEP2_32(ht_entry_type, entry, 1, 0);
    ...
    bucket_loop.LoopEnd(codegen->CreateICmpULT(bucket_num, num_buckets), {bucket_num});
  }
}

void HashTable::FindAll(CodeGen &codegen, llvm::Value *ht_ptr, const std::vector<Value> &key,
  ...
  lang::Loop chain_loop{codegen, end_condition, {{"iter", bucket}}};
  {
    ...
    llvm::Value *entry_hash = codegen.Load(EntryProxy::hash, entry);
    lang::If hash_match{codegen, codegen->CreateICmpEQ(entry_hash, hash), "hashMatch"};
    {
      llvm::Value *iter_keys = codegen->CreateConstInBoundsGEP1_32(entry_type, entry, 1);
      std::vector<Value> entry_keys;
      llvm::Value *data_area = key_storage_.LoadValues(codegen, iter_keys, entry_keys);
...
void HashTable::VectorizedIterate(UNUSED_ATTRIBUTE CodeGen &codegen, UNUSED_ATTRIBUTE llvm::Value *ht_ptr,
                                  UNUSED_ATTRIBUTE Vector &selection_vector,
                                  UNUSED_ATTRIBUTE HashTable::VectorizedIterateCallback &callback) const {
  throw NotImplementedException{"Vectorized iteration over hash-tables not supported yet"};
}
