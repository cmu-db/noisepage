...
// updater.cpp
//
// Identification: src/execution/updater.cpp
//
...
void Updater::Init(storage::DataTable *table, executor::ExecutorContext *executor_context, Target *target_vector,
  ...
  target_list_ = new TargetList(target_vector, target_vector + target_vector_size);
...
char *Updater::Prepare(uint32_t tile_group_id, uint32_t tuple_offset) {
  ...
  is_owner_ = TransactionRuntime::IsOwner(*txn, tile_group_header, tuple_offset);
  ...
  acquired_ownership_ = TransactionRuntime::AcquireOwnership(*txn, tile_group_header, tuple_offset);
...
void Updater::Update() {
  PELOTON_ASSERT(table_ != nullptr && executor_context_ != nullptr);
  LOG_TRACE("Updating tuple <%u, %u> from table '%s' (db ID: %u, table ID: %u)", old_location_.block,
            old_location_.offset, table_->GetName().c_str(), table_->GetDatabaseOid(), table_->GetOid());
  auto *txn = executor_context_->GetTransaction();
  ...
  ContainerTuple<storage::TileGroup> new_tuple(table_->GetTileGroupById(new_location_.block).get(),
                                               new_location_.offset);
  ItemPointer *indirection = tile_group_header->GetIndirection(old_location_.offset);
  auto result = table_->InstallVersion(&new_tuple, target_list_, txn, indirection);
  if (result == false) {
    TransactionRuntime::YieldOwnership(*txn, tile_group_header, old_location_.offset);
    return;
  }
...
void Updater::UpdatePK() {
  ...
  bool result = table_->InsertTuple(&tuple, new_location_, txn, &index_entry_ptr);
  if (result == false) {
