...
// pipeline.cpp
//
// Identification: src/execution/pipeline.cpp
//
...
std::string CreateUniqueFunctionName(Pipeline &pipeline, const std::string &prefix) {
  ...
  return StringUtil::Format("_%" PRId64 "_pipeline_%u_%s_%s", cc.GetID(), pipeline.GetId(), prefix.c_str(),
...
PipelineContext::LoopOverStates::LoopOverStates(PipelineContext &pipeline_ctx) : ctx_(pipeline_ctx) {}

void PipelineContext::LoopOverStates::Do(const std::function<void(llvm::Value *)> &body) const {
  ...
  llvm::Value *thread_states = exec_consumer.GetThreadStatesPtr(compilation_ctx);

  llvm::Value *num_threads = codegen.Load(ThreadStatesProxy::num_threads, thread_states);
  llvm::Value *state_size = codegen.Load(ThreadStatesProxy::state_size, thread_states);
  ...
  lang::Loop state_loop{codegen, loop_cond, {{"tid", tid}}};
  {
    ...
    llvm::Value *state = codegen->CreatePointerCast(raw_ptr, ctx_.GetThreadStateType()->getPointerTo());
...
void PipelineContext::LoopOverStates::DoParallel(const std::function<void(llvm::Value *)> &body) const {
  ...
  std::vector<FunctionDeclaration::ArgumentInfo> args = {{"queryState", query_state.GetType()->getPointerTo()},
                                                         {"threadState", ctx_.GetThreadStateType()->getPointerTo()}};
  FunctionDeclaration decl{codegen.GetCodeContext(), name, FunctionDeclaration::Visibility::Internal,
  ...
  std::vector<llvm::Value *> dispatch_args = {
      ...
      // The function
      codegen->CreatePointerCast(func.GetFunction(), proxy::TypeBuilder<void (*)(void *, void *)>::GetType(codegen))};
...
PipelineContext::Id PipelineContext::RegisterState(std::string name, llvm::Type *type) {
  ...
  PELOTON_ASSERT(state_components_.size() < std::numeric_limits<uint8_t>::max());
...
void PipelineContext::FinalizeState(CodeGen &codegen) {
  ...
  // Build
  thread_state_type_ = llvm::StructType::create(codegen.GetContext(), types, "ThreadState");
}

llvm::Value *PipelineContext::AccessThreadState(UNUSED_ATTRIBUTE CodeGen &codegen) const {
  PELOTON_ASSERT(thread_state_ != nullptr);
  return thread_state_;
}

llvm::Value *PipelineContext::LoadFlag(CodeGen &codegen) const { return LoadState(codegen, init_flag_id_); }

void PipelineContext::StoreFlag(CodeGen &codegen, llvm::Value *flag) const {
  PELOTON_ASSERT(flag->getType()->isIntegerTy(1) && flag->getType() == codegen.BoolType());
  auto *flag_ptr = LoadStatePtr(codegen, init_flag_id_);
  codegen->CreateStore(flag, flag_ptr);
}

void PipelineContext::MarkInitialized(CodeGen &codegen) const { StoreFlag(codegen, codegen.ConstBool(true)); }

llvm::Value *PipelineContext::LoadStatePtr(CodeGen &codegen, PipelineContext::Id state_id) const {
  auto name = state_components_[state_id].first + "Ptr";
  return codegen->CreateConstInBoundsGEP2_32(GetThreadStateType(), AccessThreadState(codegen), 0, state_id, name);
}

llvm::Value *PipelineContext::LoadState(CodeGen &codegen, PipelineContext::Id state_id) const {
  auto name = state_components_[state_id].first;
  return codegen->CreateLoad(LoadStatePtr(codegen, state_id), name);
}

uint32_t PipelineContext::GetEntryOffset(CodeGen &codegen, PipelineContext::Id state_id) const {
...
bool PipelineContext::HasState() const {
  PELOTON_ASSERT(thread_state_type_ != nullptr && "Cannot query state components until it has been finalized");
...
Pipeline::Pipeline(CompilationContext &compilation_ctx)
    : compilation_ctx_(compilation_ctx), pipeline_index_(0), parallelism_(Pipeline::Parallelism::Flexible) {
  id_ = compilation_ctx.RegisterPipeline(*this);
}

Pipeline::Pipeline(OperatorTranslator *translator, Pipeline::Parallelism parallelism)
...
void Pipeline::MarkSource(OperatorTranslator *translator, Pipeline::Parallelism parallelism) {
  PELOTON_ASSERT(translator == pipeline_.back());

  // Check parallel execution settings
  bool parallel_exec_disabled = !settings::SettingsManager::GetBool(settings::SettingId::parallel_execution);
  ...
  if (parallel_exec_disabled || !parallel_consumer || parallelism == Pipeline::Parallelism::Serial ||
...
void Pipeline::InstallStageBoundary(UNUSED_ATTRIBUTE const OperatorTranslator *translator) {
...
bool Pipeline::AtStageBoundary() const {
  return std::find(stage_boundaries_.begin(), stage_boundaries_.end(), pipeline_index_) != stage_boundaries_.end();
}

uint32_t Pipeline::GetNumStages() const { return static_cast<uint32_t>(stage_boundaries_.size()) + 1; }

uint32_t Pipeline::GetTranslatorStage(const OperatorTranslator *translator) const {
...
std::string Pipeline::ConstructPipelineName() const {
  ...
  for (auto riter = pipeline_.rbegin(), rend = pipeline_.rend(); riter != rend; ++riter) {
...
void Pipeline::InitializePipeline(PipelineContext &pipeline_ctx) {
  ...
  auto thread_state_size = static_cast<uint32_t>(codegen.SizeOf(pipeline_ctx.GetThreadStateType()));

  // Setup thread states
  llvm::Value *thread_states = consumer.GetThreadStatesPtr(compilation_ctx_);
  codegen.Call(ThreadStatesProxy::Reset, {thread_states, codegen.Const32(thread_state_size)});
  ...
  {
    PipelineContext::SetState state_access{pipeline_ctx, init_func.GetArgumentByPosition(1)};
...
void Pipeline::RunSerial(const std::function<void(ConsumerContext &)> &body) {
  Run(nullptr, {}, {},
      [&body](ConsumerContext &ctx, UNUSED_ATTRIBUTE const std::vector<llvm::Value *> &args) { body(ctx); });
}

void Pipeline::RunParallel(llvm::Function *dispatch_func, const std::vector<llvm::Value *> &dispatch_args,
...
void Pipeline::Run(llvm::Function *dispatch_func, const std::vector<llvm::Value *> &dispatch_args,
...
void Pipeline::DoRun(PipelineContext &pipeline_ctx, llvm::Function *dispatch_func,
  ...
  // Function signature
  std::string func_name = CreateUniqueFunctionName(*this, IsParallel() ? "parallelWork" : "serialWork");
  auto visibility = FunctionDeclaration::Visibility::Internal;
  auto *ret_type = codegen.VoidType();
  std::vector<FunctionDeclaration::ArgumentInfo> args = {{"queryState", query_state.GetType()->getPointerTo()},
  ...
  FunctionBuilder func{cc, declaration};
  {
    ...
    if (IsParallel()) {
      thread_state = codegen->CreatePointerCast(thread_state, pipeline_ctx.GetThreadStateType()->getPointerTo());
  ...
  invoke_args.insert(invoke_args.end(), dispatch_args.begin(), dispatch_args.end());

  if (dispatch_func != nullptr) {
    // Convert QueryState to void *
    invoke_args[0] = codegen->CreateBitOrPointerCast(invoke_args[0], codegen.VoidPtrType());
    // Tag on the pipeline function
    invoke_args.push_back(codegen->CreateBitCast(func.GetFunction(), codegen.VoidPtrType()));
