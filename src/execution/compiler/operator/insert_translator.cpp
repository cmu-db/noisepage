...
InsertTranslator::InsertTranslator(const planner::InsertPlanNode &plan, CompilationContext *compilation_context, Pipeline *pipeline)
    : OperatorTranslator(plan, compilation_context, pipeline, brain::ExecutionOperatingUnitType::INSERT),
      ...
      table_pm_(GetCodeGen()->GetCatalogAccessor()->GetTable(GetPlanAs<planner::InsertPlanNode>().GetTableOid())
...
void InsertTranslator::PerformPipelineWork(WorkContext *context, FunctionBuilder *function) const {
  ...
  for (uint32_t idx = 0; idx < GetPlanAs<planner::InsertPlanNode>().GetBulkInsertCount(); idx++) {
    ...
    // Insert into each index.
    const auto &indexes = GetCodeGen()->GetCatalogAccessor()->GetIndexOids(GetPlanAs<planner::InsertPlanNode>().GetTableOid());
    for (auto &index_oid : indexes) {
...
void InsertTranslator::DeclareInserter(terrier::execution::compiler::FunctionBuilder *builder) const {
  ...
  ast::Expr *inserter_setup = GetCodeGen()->StorageInterfaceInit(inserter_, GetExecutionContext(),
      !GetPlanAs<planner::InsertPlanNode>().GetTableOid(), col_oids_, true);
...
void InsertTranslator::GenInserterFree(terrier::execution::compiler::FunctionBuilder *builder) const {
  ...
  ast::Expr *inserter_free = GetCodeGen()->CallBuiltin(ast::Builtin::StorageInterfaceFree,
...
void InsertTranslator::GenIndexInsert(WorkContext *context, FunctionBuilder *builder, const catalog::index_oid_t &index_oid) const {
...
//  pr_filler_.GenFiller(index_pm, index_schema, GetCodeGen()->MakeExpr(insert_index_pr), builder);
  ...
  for (const auto &index_col : index_schema.GetColumns()) {
    ...
    auto set_key_call = GetCodeGen()->PRSet(GetCodeGen()->MakeExpr(insert_index_pr), attr_type, nullable, attr_offset,
                                            col_expr, true);
