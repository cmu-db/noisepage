...
ast::Expr *CSVScanTranslator::GetTableColumn(catalog::col_oid_t col_oid) const {
  const auto output_schema = GetPlan().GetOutputSchema();
  if (col_oid.underlying_value() > output_schema->NumColumns()) {
    throw EXECUTION_EXCEPTION(
        fmt::format("Codegen: out-of-bounds CSV column access @ idx={}", col_oid.underlying_value()));
  }

  // Return the field converted to the appropriate type.
  auto *codegen = GetCodeGen();
  auto *field = GetField(col_oid.underlying_value());
  auto output_type = sql::GetTypeId(GetPlan().GetOutputSchema()->GetColumn(col_oid.underlying_value()).GetType());
  switch (output_type) {
