...
void IndexCreateTranslator::IndexInsert(WorkContext *ctx, FunctionBuilder *function) const {
  ...
  for (const auto &index_col : index_schema.GetColumns()) {
    ...
    if (sql_type == sql::TypeId::Decimal) {
      ...
      col_expr = GetCodeGen()->DecimalSetScale(vpi_get_expr, tbl_col.TypeModifier());
    } else {
    ...
    auto *set_key_call =
        codegen_->PRSet(index_pr_expr, attr_type, nullable, attr_offset, col_expr, false, index_col.TypeModifier());
