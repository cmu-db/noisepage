...
catalog::Schema CteScanTranslator::GetPlanSchema() const {
  return *GetPlanAs<planner::CteScanPlanNode>().GetTableSchema();
}
...
ast::Expr *CteScanTranslator::GetTableColumn(catalog::col_oid_t col_oid) const {
  return SeqScanTranslator::GetTableColumn(col_oid);
}
ast::Expr *CteScanTranslator::TableIterInitExpr() const { return GetCodeGen()->TempTableIterInit(tvi_var_,
                                                                                          GetCteScanIterator(),
                                                                                                 col_oids_var_,
                                                                                          GetCompilationContext()->GetExecutionContextPtrFromQueryState()); }
