...
ast::Expr* HashJoinLeftTranslator::GetOutput(uint32_t attr_idx) {
...
ast::Expr* HashJoinRightTranslator::GetProbeValue(uint32_t idx) {
  ...
  if (is_child_materializer_) {
    return prev_translator_->GetOutput(idx);
  }
...
void HashJoinRightTranslator::GenProbeLoop(FunctionBuilder * builder) {
  ...
  if (is_child_materializer_) {
    auto child_tuple = prev_translator_->GetMaterializedTuple();
