...
, codegen_(query) {
  // Make the pipelines
  Pipeline * main_pipeline = new (query_->GetRegion()) Pipeline(codegen_);
  MakePipelines(query_->GetPlan(), main_pipeline);
  auto *output_translator = new (query->GetRegion()) OutputTranslator(&codegen_);
...
void Compiler::Compile() {
  ...
  // Compile
  ast::File* compiled_file = codegen_.Compile(std::move(decls));
...
void Compiler::GenStateStruct(tpl::util::RegionVector<tpl::ast::Decl *> *top_level,
  ...
  ast::Identifier dummy_type = codegen_.BuiltinType(ast::BuiltinType::Kind::Int32);
...
void Compiler::GenHelperStructsAndFunctions(tpl::util::RegionVector<tpl::ast::Decl *> *top_level,
  ...
  top_level->insert(top_level->begin(), decls.begin(), decls.end());
}
...
void Compiler::MakePipelines(const terrier::planner::AbstractPlanNode & op, Pipeline * curr_pipeline) {
  switch (op.GetPlanNodeType()) {
    ...
    case terrier::planner::PlanNodeType::ORDERBY: {
      ...
      Pipeline *next_pipeline = new(query_->GetRegion()) Pipeline(&codegen_);
    ...
    case terrier::planner::PlanNodeType::HASHJOIN: {
      ...
      auto *next_pipeline = new(query_->GetRegion()) Pipeline(&codegen_);
