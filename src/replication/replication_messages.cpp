...
common::json BaseReplicationMessage::ToJson() const {
  common::json json;
  json[key_message_type] = ReplicationMessageTypeToString(GetMessageType());
  json[key_metadata] = metadata_.ToJson();
  return json;
}

BaseReplicationMessage::BaseReplicationMessage(const common::json &json)
    : metadata_(ReplicationMessageMetadata(json[key_metadata])) {}

BaseReplicationMessage::BaseReplicationMessage(ReplicationMessageMetadata metadata) : metadata_(metadata) {}
...
AckMsg::AckMsg(const common::json &json) : BaseReplicationMessage(json), message_ack_id_(json[key_message_ack_id]) {}

AckMsg::AckMsg(ReplicationMessageMetadata metadata, msg_id_t message_ack_id)
    : BaseReplicationMessage(metadata), message_ack_id_(message_ack_id) {}
...
RecordsBatchMsg::RecordsBatchMsg(ReplicationMessageMetadata metadata, record_batch_id_t batch_id,
    ...
    : BaseReplicationMessage(metadata),
      batch_id_(batch_id),
...
TxnAppliedMsg::TxnAppliedMsg(const common::json &json)
    : BaseReplicationMessage(json), applied_txn_id_(json[key_applied_txn_id]) {}

TxnAppliedMsg::TxnAppliedMsg(ReplicationMessageMetadata metadata, transaction::timestamp_t applied_txn_id)
    : BaseReplicationMessage(metadata), applied_txn_id_(applied_txn_id) {}

// BaseReplicationMessage switches on the json's key_message_type to figure out what type of message to create.

BaseReplicationMessage BaseReplicationMessage::ParseFromJson(const common::json &json) {
  ReplicationMessageType msg_type = ReplicationMessageTypeFromString(json[key_message_type]);
  switch (msg_type) {
      // clang-format off
    case ReplicationMessageType::ACK:                 { return AckMsg(json); }
    case ReplicationMessageType::RECORDS_BATCH:       { return RecordsBatchMsg(json); }
    case ReplicationMessageType::TXN_APPLIED:         { return TxnAppliedMsg(json); }
