...
void TransactionManager::LogCommit(TransactionContext *const txn, const timestamp_t commit_time,
  ...
  if (log_manager_ != DISABLED && txn->GetRetentionPolicy() == RetentionPolicy::ENABLE_LOG) {
  ...
  txn->redo_buffer_.Finalize(true, txn->GetRetentionPolicy());
}
...
timestamp_t TransactionManager::Commit(TransactionContext *const txn, transaction::callback_fn callback,
  ...
  if (log_manager_ != DISABLED && !txn->IsReadOnly() && txn->GetRetentionPolicy() == RetentionPolicy::ENABLE_LOG) {
...
void TransactionManager::LogAbort(TransactionContext *const txn) {
  ...
  if (log_manager_ != DISABLED && txn->redo_buffer_.HasFlushed() && txn->GetRetentionPolicy() == RetentionPolicy::ENABLE_LOG) {
    ...
    byte *const abort_record = txn->redo_buffer_.NewEntry(storage::AbortRecord::Size(), txn->GetRetentionPolicy());
    ...
    // not yet logged out
    txn->redo_buffer_.Finalize(false, txn->GetRetentionPolicy());
