...
void TransactionManager::LogCommit(TransactionContext *const txn, const timestamp_t commit_time,
  ...
  if (log_manager_ != DISABLED &&
    ...
    if (!wal_async_commit_enable_) {
      ...
      // Asynchronous commit allows us to invoke the callback immediately to ack COMMIT before txn (or it's
      ...
      if (txn->IsReadOnly()) {
        ...
        byte *const commit_record =
            txn->redo_buffer_.NewEntry(storage::CommitRecord::Size(), txn->GetRetentionPolicy());
