...
std::unique_ptr<DropIndexPlanNode> DropIndexPlanNode::Builder::Build() {
  return std::unique_ptr<DropIndexPlanNode>(
      new DropIndexPlanNode(std::move(children_), std::move(output_schema_), index_oid_, cardinality_));
}
...
DropIndexPlanNode::DropIndexPlanNode(std::vector<std::unique_ptr<AbstractPlanNode>> &&children,
                                     ...
                                     int cardinality)
    : AbstractPlanNode(std::move(children), std::move(output_schema), cardinality), index_oid_(index_oid) {}
