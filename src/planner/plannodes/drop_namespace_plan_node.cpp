...
std::unique_ptr<DropNamespacePlanNode> DropNamespacePlanNode::Builder::Build() {
  return std::unique_ptr<DropNamespacePlanNode>(
      new DropNamespacePlanNode(std::move(children_), std::move(output_schema_), namespace_oid_, cardinality_));
}

DropNamespacePlanNode::DropNamespacePlanNode(std::vector<std::unique_ptr<AbstractPlanNode>> &&children,
                                             std::unique_ptr<OutputSchema> output_schema,
                                             catalog::namespace_oid_t namespace_oid, int cardinality)
    : AbstractPlanNode(std::move(children), std::move(output_schema), cardinality), namespace_oid_(namespace_oid) {}
