...
MaterializedColumnsInitializer::MaterializedColumnsInitializer(const terrier::storage::BlockLayout &layout,
                                                               ...
                                                               uint32_t num_tuples)
    : num_tuples_(num_tuples), col_ids_(std::move(col_ids)), offsets_(col_ids_.size()) {
  ...
  size_ += sizeof(TupleSlot) * num_tuples_;
...
MaterializedColumns* MaterializedColumnsInitializer::Initialize(void *head) const {
  ...
  result->size_ = size_;
  result->num_tuples_ = num_tuples_;
  ...
  result->TupleSlots()[0] = TupleSlot::INVALID;
  return result;
}
}  // namespace terrier::storage