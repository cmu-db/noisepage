...
SqlTable::SqlTable(BlockStore *const store, catalog::Schema schema, const catalog::table_oid_t oid)
    : block_store_(store), oid_(oid), schema_(std::move(schema)) {
  ...
  table_ = {new DataTable(block_store_, layout, layout_version_t(0)), layout, col_map, inv_col_map};
}
