...
void LogConsumer::PersistAllBuffers() {
...
void LogConsumer::LogConsumerLoop() {
  ...
  while (log_manager_->run_log_consumer_thread_) {
    ...
    // If the log manager has signaled to persist the buffers, persist all the filled buffers.
