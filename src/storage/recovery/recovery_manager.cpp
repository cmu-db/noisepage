...
void RecoveryManager::UpdateIndexesOnTable(transaction::TransactionContext *txn, catalog::db_oid_t db_oid,
  ...
  switch (table_oid.underlying_value()) {
    case (catalog::postgres::DATABASE_TABLE_OID.underlying_value()): {
    ...
    case (catalog::postgres::NAMESPACE_TABLE_OID.underlying_value()): {
    ...
    case (catalog::postgres::CLASS_TABLE_OID.underlying_value()): {
    ...
    case (catalog::postgres::COLUMN_TABLE_OID.underlying_value()): {
    ...
    case (catalog::postgres::CONSTRAINT_TABLE_OID.underlying_value()): {
    ...
    case (catalog::postgres::INDEX_TABLE_OID.underlying_value()): {
    ...
    case (catalog::postgres::TYPE_TABLE_OID.underlying_value()): {
    ...
    case (catalog::postgres::LANGUAGE_TABLE_OID.underlying_value()): {
    ...
    case (catalog::postgres::PRO_TABLE_OID.underlying_value()): {
    ...
    if (next_record->RecordType() == LogRecordType::REDO) {  // next record is a redo record
      ...
      if (next_redo_record->GetDatabaseOid() == delete_record->GetDatabaseOid() &&
        ...
        if (db_oid == next_db_oid) {
          ...
          *(reinterpret_cast<uint32_t *>(pr->AccessForceNotNull(0))) = next_db_oid.underlying_value();
          std::vector<TupleSlot> tuple_slot_result;
          pg_database_oid_index->ScanKey(*txn, *pr, &tuple_slot_result);
          TERRIER_ASSERT(tuple_slot_result.size() == 1, "Index scan should only yield one result");
...
uint32_t RecoveryManager::ProcessSpecialCasePGClassRecord(
  ...
  if (curr_record->RecordType() == LogRecordType::REDO) {
    ...
    switch (updated_pg_class_oid.underlying_value()) {
      case (catalog::postgres::REL_NEXTCOLOID_COL_OID.underlying_value()): {  // Case 1
        ReplayRedoRecord(txn, curr_record);
        return 0;  // No additional logs processed
      }

      case (catalog::postgres::REL_SCHEMA_COL_OID.underlying_value()): {  // Case 2
        // TODO(Gus): Add support for recovering DDL changes.
        return 0;  // No additional logs processed
      }

      case (catalog::postgres::REL_PTR_COL_OID.underlying_value()): {  // Case 3
...
storage::index::Index *RecoveryManager::GetCatalogIndex(
  ...
  TERRIER_ASSERT((oid.underlying_value()) < catalog::START_OID, "Oid must be a valid catalog oid");

  switch (oid.underlying_value()) {
    case (catalog::postgres::NAMESPACE_OID_INDEX_OID.underlying_value()): {
      return db_catalog->namespaces_oid_index_;
    }

    case (catalog::postgres::NAMESPACE_NAME_INDEX_OID.underlying_value()): {
      return db_catalog->namespaces_name_index_;
    }

    case (catalog::postgres::CLASS_OID_INDEX_OID.underlying_value()): {
      return db_catalog->classes_oid_index_;
    }

    case (catalog::postgres::CLASS_NAME_INDEX_OID.underlying_value()): {
      return db_catalog->classes_name_index_;
    }

    case (catalog::postgres::CLASS_NAMESPACE_INDEX_OID.underlying_value()): {
      return db_catalog->classes_namespace_index_;
    }

    case (catalog::postgres::INDEX_OID_INDEX_OID.underlying_value()): {
      return db_catalog->indexes_oid_index_;
    }

    case (catalog::postgres::INDEX_TABLE_INDEX_OID.underlying_value()): {
      return db_catalog->indexes_table_index_;
    }

    case (catalog::postgres::COLUMN_OID_INDEX_OID.underlying_value()): {
      return db_catalog->columns_oid_index_;
    }

    case (catalog::postgres::COLUMN_NAME_INDEX_OID.underlying_value()): {
      return db_catalog->columns_name_index_;
    }

    case (catalog::postgres::TYPE_OID_INDEX_OID.underlying_value()): {
      return db_catalog->types_oid_index_;
    }

    case (catalog::postgres::TYPE_NAME_INDEX_OID.underlying_value()): {
      return db_catalog->types_name_index_;
    }

    case (catalog::postgres::TYPE_NAMESPACE_INDEX_OID.underlying_value()): {
      return db_catalog->types_namespace_index_;
    }

    case (catalog::postgres::CONSTRAINT_OID_INDEX_OID.underlying_value()): {
      return db_catalog->constraints_oid_index_;
    }

    case (catalog::postgres::CONSTRAINT_NAME_INDEX_OID.underlying_value()): {
      return db_catalog->constraints_name_index_;
    }

    case (catalog::postgres::CONSTRAINT_NAMESPACE_INDEX_OID.underlying_value()): {
      return db_catalog->constraints_namespace_index_;
    }

    case (catalog::postgres::CONSTRAINT_TABLE_INDEX_OID.underlying_value()): {
      return db_catalog->constraints_table_index_;
    }

    case (catalog::postgres::CONSTRAINT_INDEX_INDEX_OID.underlying_value()): {
      return db_catalog->constraints_index_index_;
    }

    case (catalog::postgres::CONSTRAINT_FOREIGNTABLE_INDEX_OID.underlying_value()): {
      return db_catalog->constraints_foreigntable_index_;
    }

    case (catalog::postgres::LANGUAGE_OID_INDEX_OID.underlying_value()): {
      return db_catalog->languages_oid_index_;
    }

    case (catalog::postgres::LANGUAGE_NAME_INDEX_OID.underlying_value()): {
      return db_catalog->languages_name_index_;
    }

    case (catalog::postgres::PRO_OID_INDEX_OID.underlying_value()): {
      return db_catalog->procs_oid_index_;
    }

    case (catalog::postgres::PRO_NAME_INDEX_OID.underlying_value()): {
