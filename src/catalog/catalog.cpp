...
void Catalog::BootstrapDatabase(transaction::TransactionContext *txn, db_oid_t db_oid) {
  ...
  std::vector<CatalogTableType> c_tables = {DATABASE, TABLESPACE, ATTRIBUTE, NAMESPACE, CLASS,
                                            TYPE,     ATTRDEF,    SETTINGS,  INDEX};
...
void Catalog::CreatePGIndex(terrier::transaction::TransactionContext *txn, terrier::catalog::db_oid_t db_oid) {
  IndexCatalogTable::Create(txn, this, db_oid, "pg_index");
}
...
void Catalog::Dump(transaction::TransactionContext *txn, const db_oid_t db_oid) {
  ...
  auto index_handle = db_handle.GetIndexTable(txn, db_oid);
