...
std::unique_ptr<optimizer::ColumnStatsBase> PgStatisticImpl::GetColumnStatistics(
  ...
  switch (type) {
    case type::TypeId::BOOLEAN: {
      ...
      return std::make_unique<optimizer::NewColumnStats<T>>(db_oid_, table_oid, col_oid, num_rows, null_rows,
    ...
    switch (type) {
      case type::TypeId::BOOLEAN: {
        ...
        col_stats_list.push_back(std::make_unique<optimizer::NewColumnStats<T>>(
            db_oid_, table_oid, col_oid, num_rows, null_rows, std::move(top_k), std::move(histogram)));
        break;
      }
      case type::TypeId::TINYINT:
      case type::TypeId::SMALLINT:
