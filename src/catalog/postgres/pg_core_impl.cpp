...
void PgCoreImpl::BootstrapPRIsPgStatistic() {
...
void PgCoreImpl::BootstrapPgStatistic(common::ManagedPointer<transaction::TransactionContext> txn,
                          common::ManagedPointer<DatabaseCatalog> dbc) {
...
bool PgCoreImpl::DeleteTable(const common::ManagedPointer<transaction::TransactionContext> txn,
  ...
  {
    auto result = DeleteColumnStatistics<Schema::Column>(txn, table);
    if (!result) return false;
  }
...
template <typename Column, typename ClassOid, typename ColOid>
void PgCoreImpl::CreateColumnStatistic(const common::ManagedPointer<transaction::TransactionContext> txn,
                                            const ClassOid class_oid, const ColOid col_oid, const Column &col) {
...
template <typename Column, typename ClassOid>
bool PgCoreImpl::DeleteColumnStatistics(const common::ManagedPointer<transaction::TransactionContext> txn,
                                             const ClassOid class_oid) {
  const auto &oid_pri = statistics_oid_index_->GetProjectedRowInitializer();
