...
std::vector<col_oid_t> AttributeCatalogTable::GetTableColumns(transaction::TransactionContext *txn, table_oid_t table) {
  ...
  while (it != end) {
    ...
    if (*(reinterpret_cast<uint32_t *>(col_p)) == !table) {
