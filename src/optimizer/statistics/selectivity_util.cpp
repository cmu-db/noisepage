...
double SelectivityUtil::ComputeSelectivity(common::ManagedPointer<ColumnStats> column_stats,
...
double SelectivityUtil::LessThan(common::ManagedPointer<ColumnStats> column_stats,
                                 const ValueCondition &condition) {
  ...
  double SelectivityUtil::Equal(common::ManagedPointer<ColumnStats> column_stats, const ValueCondition &condition) {
    // Convert value type to raw value (double)
    TERRIER_ASSERT(condition.GetPointerToValue()->GetReturnValueType() == type::TypeId::DECIMAL,
                   "It seems like there's an assumption that it's a DECIMAL type.");
    const auto value = condition.GetPointerToValue()->Peek<double>();
  ...
  double SelectivityUtil::Like(common::ManagedPointer<ColumnStats> column_stats, const ValueCondition &condition) {
