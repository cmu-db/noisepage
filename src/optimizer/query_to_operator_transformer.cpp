...
namespace terrier {
namespace optimizer {
QueryToOperatorTransformer::QueryToOperatorTransformer(concurrency::TransactionContext *txn)
    : txn_(txn), get_id(0), enable_predicate_push_down_(settings::SettingsManager::GetBool(settings::SettingId::predicate_push_down)) {}
    
std::shared_ptr<OperatorExpression>QueryToOperatorTransformer::ConvertToOpExpression(parser::SQLStatement *op) {
...
void QueryToOperatorTransformer::Visit(parser::SelectStatement *op) {
  ...
  if (RequireAggregation(op)) {
    ...
    if (op->group_by == nullptr) {
      ...
      auto group_by_cols =
          std::vector<std::shared_ptr<parser::expression::AbstractExpression>>(
              num_group_by_cols);
      for (size_t i = 0; i < num_group_by_cols; i++) {
        group_by_cols[i] = std::shared_ptr<parser::expression::AbstractExpression>(
  ...
  if (op->limit != nullptr) {
    std::vector<parser::expression::AbstractExpression *> sort_exprs;
    std::vector<bool> sort_ascending;

    if (op->order != nullptr) {
      const auto &order_info = op->order;
      for (auto &expr : order_info->exprs) { sort_exprs.push_back(expr.get()); }
      for (auto &type : order_info->types) { sort_ascending.push_back(type == parser::kOrderAsc); }
    }
    auto limit_expr = std::make_shared<OperatorExpression>(LogicalLimit::make(op->limit->offset, op->limit->limit, std::move(sort_exprs), std::move(sort_ascending)));
...
void QueryToOperatorTransformer::Visit(parser::InsertStatement *op) {
  std::shared_ptr<catalog::TableCatalogEntry> target_table = catalog::Catalog::GetInstance()->GetTableCatalogEntry(txn_, op->GetDatabaseName(), op->GetSchemaName(), op->GetTableName());
...
void QueryToOperatorTransformer::Visit(parser::expression::ComparisonExpression *expr) {
...
void QueryToOperatorTransformer::Visit(parser::expression::OperatorExpression *expr) {
...
bool QueryToOperatorTransformer::RequireAggregation(const parser::SelectStatement *op) {
  ...
  for (auto &expr : op->getSelectList()) {
    std::vector<parser::expression::AggregateExpression *> aggr_exprs;
    parser::expression::ExpressionUtil::GetAggregateExprs(aggr_exprs, expr.get());
    if (!aggr_exprs.empty()) {
...
bool QueryToOperatorTransformer::IsSupportedConjunctivePredicate(parser::expression::AbstractExpression *expr) {
...
bool QueryToOperatorTransformer::IsSupportedSubSelect(
  ...
  std::vector<parser::expression::AbstractExpression *> predicates;
...
bool QueryToOperatorTransformer::GenerateSubquerytree(
    parser::expression::AbstractExpression *expr, oid_t child_id, bool single_join) {
  ...
  auto sub_select =
      static_cast<const parser::expression::SubqueryExpression *>(subquery_expr)
          ->GetSubSelect()
          .get();
  ...
  std::vector<parser::expression::AbstractExpression *> select_list;
  // Construct join
