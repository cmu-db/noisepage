...
void QueryToOperatorTransformer::Visit(common::ManagedPointer<parser::SelectStatement> op) {
  ...
  if (op->GetSelectTable() != nullptr) {
    ...
    // SELECT without FROM
    output_expr_ = std::make_unique<OperatorNode>(LogicalGet::Make(context_->GetNextPlanNodeID()),
  ...
  if (!predicates_.empty()) {
    auto filter_expr =
        std::make_unique<OperatorNode>(LogicalFilter::Make(std::move(predicates_), context_->GetNextPlanNodeID()),
                                       std::vector<std::unique_ptr<OperatorNode>>{});
  ...
  if (QueryToOperatorTransformer::RequireAggregation(common::ManagedPointer(op))) {
    ...
    if (op->GetSelectGroupBy() == nullptr) {
      ...
      agg_expr = std::make_unique<OperatorNode>(LogicalAggregateAndGroupBy::Make(context_->GetNextPlanNodeID()),
                                                std::vector<std::unique_ptr<OperatorNode>>{});
      ...
      agg_expr = std::make_unique<OperatorNode>(
          LogicalAggregateAndGroupBy::Make(std::move(group_by_cols), context_->GetNextPlanNodeID()),
          std::vector<std::unique_ptr<OperatorNode>>{});
      agg_expr->PushChild(std::move(output_expr_));
      output_expr_ = std::move(agg_expr);
      ...
      if (!having.empty()) {
        auto filter_expr =
            std::make_unique<OperatorNode>(LogicalFilter::Make(std::move(having), context_->GetNextPlanNodeID()),
                                           std::vector<std::unique_ptr<OperatorNode>>{});
        filter_expr->PushChild(std::move(output_expr_));
        output_expr_ = std::move(filter_expr);
      }
    ...
    output_expr_ = std::make_unique<OperatorNode>(
        LogicalAggregateAndGroupBy::Make(std::move(group_by_cols), context_->GetNextPlanNodeID()), std::move(c));
  }
  ...
  switch (node->GetJoinType()) {
    case parser::JoinType::INNER: {
      join_expr = std::make_unique<OperatorNode>(
          LogicalInnerJoin::Make(std::move(join_predicates), context_->GetNextPlanNodeID()),
          std::vector<std::unique_ptr<OperatorNode>>{});
    ...
    case parser::JoinType::OUTER: {
      join_expr = std::make_unique<OperatorNode>(
          LogicalOuterJoin::Make(std::move(join_predicates), context_->GetNextPlanNodeID()),
          std::vector<std::unique_ptr<OperatorNode>>{});
      join_expr->PushChild(std::move(left_expr));
      join_expr->PushChild(std::move(right_expr));
      break;
    ...
    case parser::JoinType::LEFT: {
      join_expr = std::make_unique<OperatorNode>(
          LogicalLeftJoin::Make(std::move(join_predicates), context_->GetNextPlanNodeID()),
          std::vector<std::unique_ptr<OperatorNode>>{});
      join_expr->PushChild(std::move(left_expr));
      join_expr->PushChild(std::move(right_expr));
      break;
    ...
    case parser::JoinType::RIGHT: {
      join_expr = std::make_unique<OperatorNode>(
          LogicalRightJoin::Make(std::move(join_predicates), context_->GetNextPlanNodeID()),
          std::vector<std::unique_ptr<OperatorNode>>{});
      join_expr->PushChild(std::move(left_expr));
      join_expr->PushChild(std::move(right_expr));
      break;
    ...
    case parser::JoinType::SEMI: {
      join_expr = std::make_unique<OperatorNode>(
          LogicalSemiJoin::Make(std::move(join_predicates), context_->GetNextPlanNodeID()),
          std::vector<std::unique_ptr<OperatorNode>>{});
      join_expr->PushChild(std::move(left_expr));
      join_expr->PushChild(std::move(right_expr));
      break;
...
void QueryToOperatorTransformer::Visit(common::ManagedPointer<parser::TableRef> node) {
  ...
  if (node->GetSelect() != nullptr) {
    ...
    output_expr_ = std::make_unique<OperatorNode>(
        LogicalQueryDerivedGet::Make(table_alias, std::move(alias_to_expr_map), context_->GetNextPlanNodeID()),
        std::vector<std::unique_ptr<OperatorNode>>{});
    ...
    output_expr_ = std::make_unique<OperatorNode>(
        LogicalGet::Make(db_oid_, accessor_->GetDefaultNamespace(), accessor_->GetTableOid(node->GetTableName()), {},
                         node->GetAlias(), false, context_->GetNextPlanNodeID()),
        std::vector<std::unique_ptr<OperatorNode>>{});
  }
}
...
void QueryToOperatorTransformer::Visit(common::ManagedPointer<parser::CreateFunctionStatement> op) {
  ...
  auto create_expr = std::make_unique<OperatorNode>(
      LogicalCreateFunction::Make(catalog::INVALID_DATABASE_OID, accessor_->GetDefaultNamespace(), op->GetFuncName(),
                                  ...
                                  op->GetFuncParameters().size(), op->ShouldReplace(), context_->GetNextPlanNodeID()),
      std::vector<std::unique_ptr<OperatorNode>>{});
  ...
  switch (create_type) {
    case parser::CreateStatement::CreateType::kDatabase:
      create_expr = std::make_unique<OperatorNode>(
          LogicalCreateDatabase::Make(op->GetDatabaseName(), context_->GetNextPlanNodeID()),
          std::vector<std::unique_ptr<OperatorNode>>{});
      break;
    case parser::CreateStatement::CreateType::kTable:
      create_expr = std::make_unique<OperatorNode>(
          LogicalCreateTable::Make(accessor_->GetNamespaceOid(op->GetNamespaceName()), op->GetTableName(),
                                   op->GetColumns(), op->GetForeignKeys(), context_->GetNextPlanNodeID()),
          std::vector<std::unique_ptr<OperatorNode>>{});
    ...
    case parser::CreateStatement::CreateType::kTrigger: {
      ...
      create_expr = std::make_unique<OperatorNode>(
          LogicalCreateTrigger::Make(db_oid_, accessor_->GetDefaultNamespace(), tb_oid, op->GetTriggerName(),
                                     ...
                                     op->GetTriggerWhen(), op->GetTriggerType(), context_->GetNextPlanNodeID()),
          std::vector<std::unique_ptr<OperatorNode>>{});
      break;
    }
    case parser::CreateStatement::CreateType::kSchema:
      create_expr = std::make_unique<OperatorNode>(
          LogicalCreateNamespace::Make(op->GetNamespaceName(), context_->GetNextPlanNodeID()),
          std::vector<std::unique_ptr<OperatorNode>>{});
      break;
...
void QueryToOperatorTransformer::Visit(common::ManagedPointer<parser::DeleteStatement> op) {
  ...
  std::vector<std::unique_ptr<OperatorNode>> c;
  auto delete_expr = std::make_unique<OperatorNode>(LogicalDelete::Make(target_db_id, target_ns_id, target_table_alias,
                                                                        target_table_id, context_->GetNextPlanNodeID()),
                                                    std::move(c));
  ...
  if (op->GetDeleteCondition() != nullptr) {
    ...
    table_scan =
        std::make_unique<OperatorNode>(LogicalGet::Make(target_db_id, target_ns_id, target_table_id, predicates,
                                                        target_table_alias, true, context_->GetNextPlanNodeID()),
                                       std::vector<std::unique_ptr<OperatorNode>>{});
  } else {
    table_scan =
        std::make_unique<OperatorNode>(LogicalGet::Make(target_db_id, target_ns_id, target_table_id, {},
                                                        target_table_alias, true, context_->GetNextPlanNodeID()),
                                       std::vector<std::unique_ptr<OperatorNode>>{});
  }
...
void QueryToOperatorTransformer::Visit(common::ManagedPointer<parser::DropStatement> op) {
  ...
  switch (drop_type) {
    case parser::DropStatement::DropType::kDatabase:
      drop_expr = std::make_unique<OperatorNode>(LogicalDropDatabase::Make(db_oid_, context_->GetNextPlanNodeID()),
                                                 std::vector<std::unique_ptr<OperatorNode>>{});
      break;
    case parser::DropStatement::DropType::kTable:
      drop_expr = std::make_unique<OperatorNode>(
          LogicalDropTable::Make(accessor_->GetTableOid(op->GetTableName()), context_->GetNextPlanNodeID()),
          std::vector<std::unique_ptr<OperatorNode>>{});
      break;
    case parser::DropStatement::DropType::kIndex:
      drop_expr = std::make_unique<OperatorNode>(
          LogicalDropIndex::Make(accessor_->GetIndexOid(op->GetIndexName()), context_->GetNextPlanNodeID()),
          std::vector<std::unique_ptr<OperatorNode>>{});
      break;
    case parser::DropStatement::DropType::kSchema:
      drop_expr = std::make_unique<OperatorNode>(
          LogicalDropNamespace::Make(accessor_->GetNamespaceOid(op->GetNamespaceName()), context_->GetNextPlanNodeID()),
          std::vector<std::unique_ptr<OperatorNode>>{});
      break;
  ...
  if (op->IsFrom()) {
    ...
    auto get_op = std::make_unique<OperatorNode>(
        LogicalExternalFileGet::Make(op->GetExternalFileFormat(), op->GetFilePath(), op->GetDelimiter(),
                                     op->GetQuoteChar(), op->GetEscapeChar(), context_->GetNextPlanNodeID()),
        std::vector<std::unique_ptr<OperatorNode>>{});
...
bool QueryToOperatorTransformer::GenerateSubqueryTree(common::ManagedPointer<parser::AbstractExpression> expr,
  ...
  if (single_join) {
    op_expr = std::make_unique<OperatorNode>(LogicalSingleJoin::Make(context_->GetNextPlanNodeID()),
                                             std::vector<std::unique_ptr<OperatorNode>>{});
    op_expr->PushChild(std::move(output_expr_));
  } else {
    op_expr = std::make_unique<OperatorNode>(LogicalMarkJoin::Make(context_->GetNextPlanNodeID()),
                                             std::vector<std::unique_ptr<OperatorNode>>{});
    op_expr->PushChild(std::move(output_expr_));
  }
