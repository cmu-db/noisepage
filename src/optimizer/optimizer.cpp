...
std::unique_ptr<planner::AbstractPlanNode> Optimizer::ChooseBestPlan(
  ...
  OPTIMIZER_LOG_TRACE("Choosing best plan for group " + std::to_string(gexpr->GetGroupID().underlying_value()) +
  ...
  OPTIMIZER_LOG_TRACE("Finish Choosing best plan for group " + std::to_string(id.underlying_value()));
