...
void DeriveStats::Execute() {
  ...
  if (!children_derived_) {
    children_derived_ = true;
    ...
    for (size_t idx = 0; idx < gexpr_->GetChildrenGroupsSize(); idx++) {
      ...
      PushTask(new DeriveStats(child_group_gexpr, context_));
    }
  } else {
    StatsCalculator calculator;
    calculator.CalculateStats(gexpr_, context_->GetOptimizerContext());
