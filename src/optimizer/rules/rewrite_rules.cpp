...
void RewritePushImplicitFilterThroughJoin::Transform(common::ManagedPointer<AbstractOptimizerNode> input,
  ...
  if (!left_predicates.empty()) {
    ...
    left_branch = std::make_unique<OperatorNode>(LogicalFilter::Make(std::move(left_predicates), context->GetOptimizerContext()->GetTxn()), std::move(c),
  ...
  if (!right_predicates.empty()) {
    ...
    right_branch = std::make_unique<OperatorNode>(LogicalFilter::Make(std::move(right_predicates), context->GetOptimizerContext()->GetTxn()), std::move(c),
                                                  context->GetOptimizerContext()->GetTxn());
  } else {
  ...
  if (pushed_down) {
    ...
    auto output = std::make_unique<OperatorNode>(LogicalInnerJoin::Make(std::move(join_predicates), context->GetOptimizerContext()->GetTxn()), std::move(c),
                                                 context->GetOptimizerContext()->GetTxn());
...
void RewritePushFilterThroughAggregation::Transform(common::ManagedPointer<AbstractOptimizerNode> input,
  ...
  if (!pushdown_predicates.empty()) {
    ...
    pushdown = std::make_unique<OperatorNode>(LogicalFilter::Make(std::move(pushdown_predicates), context->GetOptimizerContext()->GetTxn()), std::move(c),
                                              context->GetOptimizerContext()->GetTxn());
  }
  ...
  auto output =
      std::make_unique<OperatorNode>(LogicalAggregateAndGroupBy::Make(std::move(cols), std::move(embedded_predicates), context->GetOptimizerContext()->GetTxn()),
...
void RewriteCombineConsecutiveFilter::Transform(common::ManagedPointer<AbstractOptimizerNode> input,
  ...
  auto output = std::make_unique<OperatorNode>(LogicalFilter::Make(std::move(root_predicates), context->GetOptimizerContext()->GetTxn()), std::move(c),
                                               context->GetOptimizerContext()->GetTxn());
  transformed->emplace_back(std::move(output));
}
...
void RewriteEmbedFilterIntoGet::Transform(common::ManagedPointer<AbstractOptimizerNode> input,
  ...
  auto output =
      std::make_unique<OperatorNode>(LogicalGet::Make(get->GetDatabaseOid(), get->GetNamespaceOid(), get->GetTableOid(),
                                                      predicates, tbl_alias, get->GetIsForUpdate(), context->GetOptimizerContext()->GetTxn()),
                                     std::move(c), context->GetOptimizerContext()->GetTxn());
  transformed->emplace_back(std::move(output));
}
...
void RewritePullFilterThroughMarkJoin::Transform(common::ManagedPointer<AbstractOptimizerNode> input,
  ...
  auto output = std::make_unique<OperatorNode>(join_children[1]->Contents(), std::move(cc), context->GetOptimizerContext()->GetTxn());
  transformed->emplace_back(std::move(output));
}
...
void RewritePullFilterThroughAggregation::Transform(common::ManagedPointer<AbstractOptimizerNode> input,
  ...
  if (!normal_predicates.empty()) {
    ...
    aggr_child = std::make_unique<OperatorNode>(LogicalFilter::Make(std::move(normal_predicates), context->GetOptimizerContext()->GetTxn()), std::move(c),
                                                context->GetOptimizerContext()->GetTxn());
  }
  ...
  auto new_aggr = std::make_unique<OperatorNode>(
      LogicalAggregateAndGroupBy::Make(std::move(new_groupby_cols), std::move(new_having), context->GetOptimizerContext()->GetTxn()), std::move(c),
      context->GetOptimizerContext()->GetTxn());

  std::vector<std::unique_ptr<AbstractOptimizerNode>> ca;
  ca.emplace_back(std::move(new_aggr));
  auto output = std::make_unique<OperatorNode>(LogicalFilter::Make(std::move(correlated_predicates), context->GetOptimizerContext()->GetTxn()), std::move(ca),
                                               context->GetOptimizerContext()->GetTxn());
  transformed->emplace_back(std::move(output));
}
