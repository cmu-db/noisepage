...
const AbstractExpression *PostgresParser::FuncCallTransform(FuncCall *root) {
  ...
  if (!IsAggregateFunction(func_name)) {
    ...
    if (root->agg_star_) {
      ...
      result = new AggregateExpression(agg_fun_type, std::move(children), root->agg_distinct_);
...
const AbstractExpression *PostgresParser::ParamRefTransform(ParamRef *root) {
  auto result = new ParameterValueExpression(root->number_ - 1);
  return result;
}
...
std::unique_ptr<SQLStatement> PostgresParser::CreateIndexTransform(IndexStmt *root) {
  ...
  for (auto cell = root->index_params_->head; cell != nullptr; cell = cell->next) {
    ...
    if (index_elem->expr_ == nullptr) {
      index_attrs.emplace_back(new IndexAttr(index_elem->name_));
    } else {
      index_attrs.emplace_back(new IndexAttr(ExprTransform(index_elem->expr_)));
    }
  }
...
std::vector<std::shared_ptr<parser::UpdateClause>> PostgresParser::UpdateTargetTransform(List *root) {
  ...
  for (auto cell = root->head; cell != nullptr; cell = cell->next) {
    ...
    auto value = ExprTransform(target->val_);
