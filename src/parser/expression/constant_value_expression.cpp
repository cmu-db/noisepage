...
ConstantValueExpression ConstantValueExpression::FromString(const std::string &val, type::TypeId type_id) {
  switch (type_id) {
    case type::TypeId::BOOLEAN: {
      if (val.empty()) return ConstantValueExpression(type_id, execution::sql::BoolVal::Null());
    ...
    case type::TypeId::INTEGER:
    case type::TypeId::BIGINT: {
      if (val.empty()) return ConstantValueExpression(type_id, execution::sql::Integer::Null());
      return ConstantValueExpression(type_id, execution::sql::Integer(std::stoll(val)));
    }
    case type::TypeId::DECIMAL: {
      if (val.empty()) return ConstantValueExpression(type_id, execution::sql::Real::Null());
      return ConstantValueExpression(type_id, execution::sql::Real(std::stod(val)));
    }
    case type::TypeId::TIMESTAMP: {
      if (val.empty()) return ConstantValueExpression(type_id, execution::sql::TimestampVal::Null());
      return ConstantValueExpression(type_id, execution::sql::TimestampVal(execution::sql::Timestamp::FromString(val)));
    }
    case type::TypeId::DATE: {
      if (val.empty()) return ConstantValueExpression(type_id, execution::sql::DateVal::Null());
