#pragma once

#include <memory>
#include <string>
#include <unordered_map>
#include <unordered_set>
#include <vector>
#include "common/sql_node_visitor.h"

namespace terrier {

namespace parser {
class ParseResult;
class SQLStatement;
class AbstractExpression;
}  // namespace parser

namespace catalog {
class CatalogAccessor;
}  // namespace catalog

namespace optimizer {
class AnnotatedExpression;
class OperatorExpression;

/**
 * @brief Transform a query from parsed statement to operator expressions.
 */
class QueryToOperatorTransformer : public SqlNodeVisitor {
 public:
  /**
   * Initialize the query to operator transformer object with a unique pointer to the catalog accessor
   * @param catalog_accessor Pointer to a catalog accessor
   */
  explicit QueryToOperatorTransformer(std::unique_ptr<catalog::CatalogAccessor> catalog_accessor);

  /**
   * Traverse the query AST to generate AST of logical operators.
   * @param op Parsed in AST tree of the SQL statement
   * @param parse_result The AST generated by the parser
   * @return Pointer to the logical operator AST
   */
  OperatorExpression *ConvertToOpExpression(common::ManagedPointer<parser::SQLStatement> op,
                                            parser::ParseResult *parse_result);

  /**
   * Traverse the select statement annotated by the binder and generate corresponding logical operator AST
   * @param node Select statement to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::SelectStatement *op, parser::ParseResult *parse_result) override;

  /**
   * Traverse the table reference object annotated by the binder and generate corresponding logical operator AST
   * @param node TableRef object to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::TableRef *, parser::ParseResult *) override;

  /**
   * Traverse the join definition annotated by the binder and generate corresponding logical operator AST
   * @param node Join definition to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::JoinDefinition *, parser::ParseResult *) override;

  /**
   * Traverse the group by description annotated by the binder and generate corresponding logical operator AST
   * @param node GroupBy description to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::GroupByDescription *, parser::ParseResult *) override;

  /**
   * Traverse the order by description annotated by the binder and generate corresponding logical operator AST
   * @param node OrderBy description to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::OrderByDescription *, parser::ParseResult *) override;

  /**
   * Traverse the limit description annotated by the binder and generate corresponding logical operator AST
   * @param node Limit description to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::LimitDescription *, parser::ParseResult *) override;

  /**
   * Traverse the create statement annotated by the binder and generate corresponding logical operator AST
   * @param node Create statement to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::CreateStatement *op, parser::ParseResult *parse_result) override;

  /**
   * Traverse the create function statement annotated by the binder and generate corresponding logical operator AST
   * @param node Create function statement to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::CreateFunctionStatement *op, parser::ParseResult *parse_result) override;

  /**
   * Traverse the insert statement annotated by the binder and generate corresponding logical operator AST
   * @param node Insert statement to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::InsertStatement *op, parser::ParseResult *parse_result) override;

  /**
   * Traverse the delete statement annotated by the binder and generate corresponding logical operator AST
   * @param node Delete statement to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::DeleteStatement *op, parser::ParseResult *parse_result) override;

  /**
   * Traverse the drop statement annotated by the binder and generate corresponding logical operator AST
   * @param node Drop statement to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::DropStatement *op, parser::ParseResult *parse_result) override;

  /**
   * Traverse the prepare statement annotated by the binder and generate corresponding logical operator AST
   * @param node Prepare statement to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::PrepareStatement *op, parser::ParseResult *parse_result) override;

  /**
   * Traverse the execute statement annotated by the binder and generate corresponding logical operator AST
   * @param node Execute statement to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::ExecuteStatement *op, parser::ParseResult *parse_result) override;

  /**
   * Traverse the trasaction statement annotated by the binder and generate corresponding logical operator AST
   * @param node Transaction statement to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::TransactionStatement *op, parser::ParseResult *parse_result) override;

  /**
   * Traverse the update statement annotated by the binder and generate corresponding logical operator AST
   * @param node Update statement to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::UpdateStatement *op, parser::ParseResult *parse_result) override;

  /**
   * Traverse the copy statement annotated by the binder and generate corresponding logical operator AST
   * @param node Copy statement to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::CopyStatement *op, parser::ParseResult *parse_result) override;

  /**
   * Traverse the analyze statement annotated by the binder and generate corresponding logical operator AST
   * @param node Analyze statement to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::AnalyzeStatement *op, parser::ParseResult *parse_result) override;

  /**
   * Traverse the comparison statement annotated by the binder and generate corresponding logical operator AST
   * @param node Comparison statement to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::ComparisonExpression *expr, parser::ParseResult *) override;

  /**
   * Traverse the operator statement annotated by the binder and generate corresponding logical operator AST
   * @param node Operator statement to be traversed
   * @param parse_result The AST generated by the parser
   */
  void Visit(parser::OperatorExpression *expr, parser::ParseResult *) override;

 private:
  /**
   * @brief Walk through an expression, split it into a set of predicates that could be joined by conjunction.
   * We need the set to perform predicate push-down. For example, for the following query
   * "SELECT test.a, test1.b FROM test, test1 WHERE test.a = test1.b and test.a = 5"
   * we could will extract "test.a = test1.b" and "test.a = 5" from the original predicate
   * and we could let "test.a = 5" to be evaluated at the table scan level
   *
   * @param expr The original predicate
   * @param parse_result The AST generated by the parser
   * @param predicates
   */
  void CollectPredicates(common::ManagedPointer<parser::AbstractExpression> expr,
                                                     parser::ParseResult *parse_result,
                                                     std::vector<AnnotatedExpression> &predicates);

  /**
   * @brief Transform a sub-query in an expression to use
   *
   * @param expr The potential sub-query expression
   * @param select_list The select list of the sub-query we generate
   * @param single_join A boolean to tell if
   *
   * @return If the expression could be transformed into sub-query, return true,
   *  return false otherwise
   */
  bool GenerateSubqueryTree(parser::AbstractExpression *expr, int child_id, parser::ParseResult *parse_result,
                            bool single_join = false);

  /**
   * @brief Decide if a conjunctive predicate is supported. We need to extract
   * conjunction predicate first then call this function to decide if the
   * predicate is supported by our system
   *
   * @param expr The conjunctive predicate provided
   *
   * @return True if supported, false otherwise
   */
  static bool IsSupportedConjunctivePredicate(common::ManagedPointer<parser::AbstractExpression> expr);

  /**
   * @brief Check if a sub-select statement is supported.
   *
   * @param op The select statement
   *
   * @return True if supported, false otherwise
   */
  static bool IsSupportedSubSelect(common::ManagedPointer<parser::SelectStatement> op);
  static bool RequireAggregation(common::ManagedPointer<parser::SelectStatement> op);

  /**
   * @brief Extract single table precates and multi-table predicates from the expr
   *
   * @param expr The original predicate
   * @param annotated_predicates The extracted conjunction predicates
   */
  void ExtractPredicates(common::ManagedPointer<parser::AbstractExpression> expr,
                                                            std::vector<AnnotatedExpression> &annotated_predicates);

  /**
   * Generate a set of table alias included in an expression
   */
  static void GenerateTableAliasSet(common::ManagedPointer<parser::AbstractExpression> expr,
                                    std::unordered_set<std::string> *table_alias_set);

  /**
   * @breif Split conjunction expression tree into a vector of expressions with AND
   */
  static void SplitPredicates(common::ManagedPointer<parser::AbstractExpression> expr,
                              std::vector<common::ManagedPointer<parser::AbstractExpression>> *predicates);

  static std::unordered_map<std::string, common::ManagedPointer<parser::AbstractExpression>> ConstructSelectElementMap(
      const std::vector<common::ManagedPointer<parser::AbstractExpression>> &select_list);

  OperatorExpression *output_expr_;

  std::unique_ptr<catalog::CatalogAccessor> accessor_;

  //  /**
  //   * @brief The Depth of the current operator, we need this to tell if there's
  //   *  dependent join in the query. Dependent join transformation logic is not
  //   *  implemented yet
  //   */
  //  int depth_;

  /**
   * @brief A set of predicates the current operator generated, we use them to
   *  generate filter operator
   */
  std::vector<AnnotatedExpression> predicates_;
};

}  // namespace optimizer
}  // namespace terrier
