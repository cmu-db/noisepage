...
class InitialCostModel : public AbstractCostModel {
 public:
  ...
  void Visit(UNUSED_ATTRIBUTE const InnerHashJoin *op) override {
    ...
    total_cost += (op_cpu_cost_ * op->GetJoinPredicates().size() + tuple_cpu_cost_) * inner_rows;
    total_cost += op_cpu_cost_ * op->GetJoinPredicates().size() * outer_rows;
