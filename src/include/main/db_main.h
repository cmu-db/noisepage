  ...
  class Builder {
   ...
   private:
    ...
    std::unique_ptr<settings::SettingsManager> BootstrapSettingsManager(const common::ManagedPointer<DBMain> db_main) {
      ...
      if (use_logging_) {
        log_file_path_ = settings_manager->GetString(settings::Param::wal_file_path);
        num_log_manager_buffers_ =
            static_cast<uint64_t>(settings_manager->GetInt64(settings::Param::wal_num_buffers));
        log_serialization_interval_ = settings_manager->GetInt(settings::Param::wal_serialization_interval);
        log_persist_interval_ = settings_manager->GetInt(settings::Param::wal_persist_interval);
