...
class TupleAccessStrategy {
 private:
  ...
  struct Block {
    ...
    MiniBlock *Column(const BlockLayout &layout, const col_id_t col_id) {
      byte *head = reinterpret_cast<byte *>(this) + AttrOffsets(layout)[col_id.underlying_value()];
 ...
 public:
  ...
  common::RawConcurrentBitmap *ColumnNullBitmap(RawBlock *block, const col_id_t col_id) const {
    TERRIER_ASSERT((col_id.underlying_value()) < layout_.NumColumns(), "Column out of bounds!");
