...
class RecoveredLog {
 public:
  ...
  explicit RecoveredLog(const std::string &log_file_name) : in_(log_file_name, std::ios::binary | std::ios::in) {}

  bool HasNext() { return in_.eof(); }
  ...
  void ReadRecord(void *position) {
    ...
    switch (type) {
      ...
      case LogRecordType::REDO: {
        ...
        record_body->tuple_slot_ = ReadValue<tuple_id_t>();
