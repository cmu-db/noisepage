...
class BlockLayout {
 public:
  ...
  uint16_t AttrSize(col_id_t col_id) const {
    ...
    return AttrSizeBytes(attr_sizes_.at(col_id.underlying_value()));
  }
   ...
   */
  bool IsVarlen(col_id_t col_id) const { return static_cast<int16_t>(attr_sizes_.at(col_id.underlying_value())) < 0; }
