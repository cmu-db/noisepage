 ...
 public:
  ...
  void WriteRowDescription(const std::vector<planner::OutputSchema::Column> &columns,
    ...
    for (uint32_t i = 0; i < columns.size(); i++) {
      ...
      const auto &name = columns[i].GetExpr()->GetAlias().empty() ? columns[i].GetName() : columns[i].GetExpr()->GetAlias();
      AppendString(name)
  ...
  void WriteDataRow(const byte *const tuple, const std::vector<planner::OutputSchema::Column> &columns,
    ...
    for (uint32_t i = 0; i < columns.size(); i++) {
      ...
      if (val->is_null_) {
        ...
        curr_offset += execution::sql::ValUtil::GetSqlSize(columns[i].GetType());
        continue;
      }
