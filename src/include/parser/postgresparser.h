...
struct ParseResult {
  ...
  void AddExpression(bool is_top_level, AbstractExpression *expr) {
    // If it is not a top level expression, it'll be owned by another expression as part of a subtree.
    if (is_top_level) {
...
class PostgresParser {
 ...
 private:
  ...
  // expressions
  static common::ManagedPointer<AbstractExpression> ExprTransform(ParseResult *result, bool is_top_level, Node *node);
