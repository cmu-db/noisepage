...
class TransactionManager {
 ...
 public:
  ...
  bool SetBufferPoolSizeLimit(uint64_t new_size) {
    return buffer_pool_->SetSizeLimit(new_size);
  }

  uint64_t GetBufferPoolSizeLimit() const {
    return buffer_pool_->GetSizeLimit();
  }
