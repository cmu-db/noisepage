...
class ContainerTuple : public AbstractTuple {
 public:
  ...
  ContainerTuple(T *container, oid_t tuple_id) : container_(container), tuple_id_(tuple_id) {}
  ...
  void SetValue(UNUSED_ATTRIBUTE oid_t column_id, UNUSED_ATTRIBUTE const type::Value &value) override {}
  ...
  char *GetData() const override { throw NotImplementedException("GetData() not supported for container tuples."); }
...
struct ContainerTupleHasher : std::unary_function<ContainerTuple<T>, std::size_t> {
  size_t operator()(const ContainerTuple<T> &tuple) const { return tuple.HashCode(); }
};
...
class ContainerTuple<std::vector<type::Value>> : public AbstractTuple {
 public:
  ...
  bool EqualsNoSchemaCheck(const ContainerTuple<std::vector<type::Value>> &other) const {
...
template <>
class ContainerTuple<execution::TileGroup> : public AbstractTuple {
 public:
  ContainerTuple(const ContainerTuple &) = default;
  ContainerTuple &operator=(const ContainerTuple &) = default;
  ContainerTuple(ContainerTuple &&) = default;
  ...
  ContainerTuple(execution::TileGroup *container, oid_t tuple_id)
      : container_(container), tuple_id_(tuple_id), column_ids_(nullptr) {}

  ContainerTuple(execution::TileGroup *container, oid_t tuple_id, const std::vector<oid_t> *column_ids)
      : container_(container), tuple_id_(tuple_id), column_ids_(column_ids) {}

  type::Value GetValue(oid_t column_id) const override {
    PELOTON_ASSERT(container_ != nullptr);
    auto col_id = (column_ids_ != nullptr ? column_ids_->at(column_id) : column_id);
