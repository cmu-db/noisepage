...
class ItemPointer {
 public:
  ...
  bool IsNull() const { return (block == INVALID_OID && offset == INVALID_OID); }
  ...
  bool operator==(const ItemPointer &rhs) const { return (block == rhs.block && offset == rhs.offset); }
...
struct ItemPointerHasher {
  size_t operator()(const ItemPointer &item) const {
    ...
    return (std::hash<oid_t>()(item.block) * 0x9ddfea08eb382d69ULL) ^ std::hash<oid_t>()(item.offset);
  }
};
