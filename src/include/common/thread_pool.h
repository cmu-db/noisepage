...
using TaskQueue = tbb::concurrent_queue<std::function<void()>>;
...
class ThreadPool {
 public:
  ...
  WorkerPool()
      : is_running_(false), busy_workers_{0} {}
  ...
  void Startup() {
    ...
    unsigned max_workers_ = std::thread::hardware_concurrency();

    // Create enough workers if necessary
    while (workers_.size() < max_workers_) {
      AddThread();
    }

    // Cut down the number of workers if we have too many workers
    if (workers_.size() > max_workers_) workers_.resize(max_workers_);
  }
 ...
 private:
  ...
  class TerrierThread {
   public:
    TerrierThread(numa_region_t numa_region, TheadPool *pool) :
        numa_region_(numa_region), pool_(pool) {}
    ~TerrierThread = default;

    //TODO(Deepayan) write GetTask - bind, RunTaskLoop - call getTask in loop with shutdown check
  ...
  uint64_t num_regions_;
  // The worker threads
  std::array<std::vector<std::thread>, numa_max_node()> workers_;
  // Flag indicating whether the pool is running
  bool is_running_;
  // The queue where workers pick up tasks
  // TODO(Deepayan) redo task queue
  std::array<TaskQueue> task_queue_;
