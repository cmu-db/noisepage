...
class MetricsManager {
 public:
   ...
   */
  ThreadLevelMetricsStore *const RegisterThread() {
    const auto thread_id = std::this_thread::get_id();
    TERRIER_ASSERT(stores_map_.Find(thread_id) == stores_map_.End(), "This thread was already registered.");
    auto *const metrics_store = new ThreadLevelMetricsStore(thread_id);
   ...
   */
  using StoresMap = common::ConcurrentMap<std::thread::id, ThreadLevelMetricsStore *const, std::hash<std::thread::id>>;
