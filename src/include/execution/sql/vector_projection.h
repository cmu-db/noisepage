...
class VectorProjection {
 public:
  ...
  VectorProjection(const std::vector<const Schema::ColumnInfo *> &col_infos, u32 size);

  /// This class cannot be copied or moved
  DISALLOW_COPY_AND_MOVE(VectorProjection);

  /// Get the metadata for the column at the given index in this projection
  const Schema::ColumnInfo *GetColumnInfo(u32 col_idx) const { return column_info_[col_idx]; }
  ...
  const u32 *GetNullBitmap(u32 col_idx) const { return column_null_bitmaps_[col_idx]; }
  ...
  void ResetColumn(const std::vector<ColumnVectorIterator> &col_iters, u32 col_idx);
  ...
  void ResetFromRaw(byte col_data[], u32 col_null_bitmap[], u32 col_idx, u32 num_tuples);
 ...
 private:
  ...
  void Setup(const std::vector<const Schema::ColumnInfo *> &col_infos, u32 size);
