...
class FunctionDecl : public Decl {
 public:
   ...
   * @param is_lambda `true` if this function is constructed from a lambda expresison
   */
  FunctionDecl(const SourcePosition &pos, Identifier name, FunctionLitExpr *func, bool is_lambda = false);
   ...
   */
  bool IsLambda() const noexcept { return is_lambda_; }
 ...
 private:
  ...
  // Is this function generated by a lambda expression.
...
class FunctionLitExpr : public Expr {
 public:
   ...
   * @param is_lambda `true` if the literal is a lambda, `false` otherwise
   */
  ...
  /**
   * @return `true` if the function is a lambda, `false` otherwise.
   */
 ...
 private:
  ...
  BlockStmt *body_;
  // Is this function literal a lambda.
  const bool is_lambda_;
};
