...
class BytecodeEmitter {
 public:
  ...
  // TODO(Amadou): Many of these emitters do not really need the bytecode to be explicitly passed in.
  // -------------------------------------------------------
  // Tables
  // -------------------------------------------------------
  /**
   ...
   * @param exec_context address of the execution context
   */
   ...
   * @param exec_context pointer to execution context
   * @param dest destination variable
   */
  void EmitOutputAlloc(Bytecode bytecode, uintptr_t exec_context, LocalVar dest);

  /**
   * Generic helper method to emit output calls.
   * @param bytecode bytecode to emit
   * @param exec_context pointer to the execution context
   */
  void EmitOutputCall(Bytecode bytecode, uintptr_t exec_context);

  /**
   * Emit code to set a column to null
   * TODO(Amadou): Remove this if it's indeed unneeded.
   * @param bytecode bytecode to set to null
   * @param exec_context pointer to the execution_context
   * @param idx index of the columns
   */
  void EmitOutputSetNull(Bytecode bytecode, uintptr_t exec_context, LocalVar idx);
   ...
   * @param exec_context pointer to the execution context
   */
  void EmitIndexIteratorInit(Bytecode bytecode, LocalVar iter, uint32_t index_oid, uintptr_t exec_context);
