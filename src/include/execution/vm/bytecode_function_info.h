...
class FunctionInfo {
 public:
  ...
  /**
   * @brief Defer an action for the current function.
   *
   * This functionality is used for TPL lambda expressions.
   * When we visit a lambda expression in the nody of the
   * current function, we defer an action that in turn visits
   ...
   * is generated by a TPL lambda, `false` otherwise.
   */
 ...
 private:
  ...
  // The captures in the event this function is a TPL lambda.
  LocalVar captures_;

  // Indicates whether this TBC function is generated by a TPL lambda.
  bool is_lambda_{false};

  // The collection of deferred actions if this function is a TPL lambda.
