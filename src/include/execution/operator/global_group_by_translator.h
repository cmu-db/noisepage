...
//
// Identification: src/include/execution/operator/global_group_by_translator.h
//
...
class GlobalGroupByTranslator : public OperatorTranslator {
 public:
  // Constructor
  GlobalGroupByTranslator(const planner::AggregatePlan &plan, CompilationContext &context, Pipeline &pipeline);
 ...
 private:
  ...
  class BufferAttributeAccess : public RowBatch::AttributeAccess {
   public:
    // Constructor
    BufferAttributeAccess(const std::vector<Value> &aggregate_vals, uint32_t agg_index)
        : aggregate_vals_(aggregate_vals), agg_index_(agg_index) {}

    Value Access(CodeGen &, RowBatch::Row &) override { return aggregate_vals_[agg_index_]; }

   private:
    // All the vals
    const std::vector<Value> &aggregate_vals_;
