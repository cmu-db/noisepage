...
class CteScanLeaderTranslator : public OperatorTranslator {
 public:
  ...
  void InitializeStateFields(util::RegionVector<ast::FieldDecl *> *state_fields) override {
    ...
    state_fields->emplace_back(codegen_->MakeField(GetCteScanIteratorVal(), cte_scan_type));
    ast::Expr *cte_scan_ptr_type = codegen_->PointerType(cte_scan_type);
    state_fields->emplace_back(codegen_->MakeField(GetCteScanIterator(), cte_scan_ptr_type));
  }
  ...
  void InitializeTeardown(util::RegionVector<ast::Stmt *> *teardown_stmts) override {
    ast::Expr *cte_free_call =
        codegen_->OneArgCall(ast::Builtin::CteScanFree, codegen_->GetStateMemberPtr(GetCteScanIteratorVal()));
