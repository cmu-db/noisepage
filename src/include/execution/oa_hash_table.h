...
// oa_hash_table.h
//
// Identification: src/include/execution/oa_hash_table.h
//
...
class OAHashTable : public HashTable {
 public:
  ...
  class OAHashTableAccess : public HashTable::HashTableAccess {
   public:
    OAHashTableAccess(const OAHashTable &hash_table, llvm::Value *ht_ptr) : hash_table_(hash_table), ht_ptr_(ht_ptr) {}

    void ExtractBucketKeys(CodeGen &codegen, llvm::Value *index, std::vector<Value> &keys) const override;

    llvm::Value *BucketValue(CodeGen &codegen, llvm::Value *index) const override;
 ...
 public:
  // Constructor
  OAHashTable();
  OAHashTable(CodeGen &codegen, const std::vector<type::Type> &key_type, uint64_t value_size);

  void Init(CodeGen &codegen, llvm::Value *ht_ptr) const override;

  llvm::Value *HashKey(CodeGen &codegen, const std::vector<Value> &key) const;

  // Generate code to handle the insertion of a new tuple
  void ProbeOrInsert(CodeGen &codegen, llvm::Value *ht_ptr, llvm::Value *hash, const std::vector<Value> &key,
  ...
  void Insert(CodeGen &codegen, llvm::Value *ht_ptr, llvm::Value *hash, const std::vector<Value> &key,
              HashTable::InsertCallback &callback) const override;

  // Generate code to iterate over the entire hash table
  void Iterate(CodeGen &codegen, llvm::Value *ht_ptr, HashTable::IterateCallback &callback) const override;

  // Generate code to iterate over the entire hash table in vectorized fashion
  void VectorizedIterate(CodeGen &codegen, llvm::Value *ht_ptr, Vector &selection_vector,
                         HashTable::VectorizedIterateCallback &callback) const override;

  // Generate code that iterates all the matches
  void FindAll(CodeGen &codegen, llvm::Value *ht_ptr, const std::vector<Value> &key,
  ...
  void PrefetchBucket(CodeGen &codegen, llvm::Value *ht_ptr, llvm::Value *hash, PrefetchType pf_type,
 ...
 private:
  ...
  ProbeResult TranslateProbing(CodeGen &codegen, llvm::Value *hash_table, llvm::Value *hash,
                               const std::vector<Value> &key, std::function<void(llvm::Value *)> key_found,
                               std::function<void(llvm::Value *)> key_not_found, bool process_value,
                               bool process_only_one_value, bool create_key_if_missing, bool return_probe_result) const;

  llvm::Value *LoadHashEntryField(CodeGen &codegen, llvm::Value *entry_ptr, uint32_t offset, uint32_t field_id) const;

  llvm::Value *PtrToInt(CodeGen &codegen, llvm::Value *ptr) const;

  llvm::Value *AdvancePointer(CodeGen &codegen, llvm::Value *ptr, uint64_t delta) const;

  llvm::Value *AdvancePointer(CodeGen &codegen, llvm::Value *ptr, llvm::Value *delta) const;

  HashTablePos GetNextEntry(CodeGen &codegen, llvm::Value *hash_table, llvm::Value *entry_ptr,
                            llvm::Value *index) const;

  llvm::Value *GetEntry(CodeGen &codegen, llvm::Value *hash_table, llvm::Value *index) const;

  HashTablePos GetEntryByHash(CodeGen &codegen, llvm::Value *hash_table, llvm::Value *hash_value) const;

  llvm::Value *GetKeyValueList(CodeGen &codegen, llvm::Value *entry_ptr) const;

  llvm::Value *IsPtrEqualTo(CodeGen &codegen, llvm::Value *value, uint64_t target) const;

  llvm::Value *IsPtrUnEqualTo(CodeGen &codegen, llvm::Value *value, uint64_t target) const;

  llvm::Value *GetKeyPtr(CodeGen &codegen, llvm::Value *entry_ptr) const;

  std::pair<llvm::Value *, llvm::Value *> GetDataCountAndPointer(CodeGen &codegen, llvm::Value *kv_p,
