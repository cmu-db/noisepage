...
class InsertPlan : public AbstractPlan {
 public:
  ...
  InsertPlan(storage::DataTable *table, std::unique_ptr<const planner::ProjectInfo> &&project_info,
             oid_t bulk_insert_count = 1)
      : target_table_(table), project_info_(std::move(project_info)), bulk_insert_count_(bulk_insert_count) {
    LOG_TRACE("Creating an Insert Plan with a projection");
  }

  // Construct with a tuple
  // This can only be handled by the interpreted exeuctor
  InsertPlan(storage::DataTable *table, std::unique_ptr<storage::Tuple> &&tuple, oid_t bulk_insert_count = 1)
      : target_table_(table), bulk_insert_count_(bulk_insert_count) {
