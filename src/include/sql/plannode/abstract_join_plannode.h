...
class AbstractJoinPlan : public AbstractPlan {
 public:
  AbstractJoinPlan(JoinType joinType, std::unique_ptr<const expression::AbstractExpression> &&predicate,
  ...
  void VisitParameters(codegen::QueryParametersMap &map, std::vector<peloton::type::Value> &values,
  ...
  const std::vector<const AttributeInfo *> &GetLeftAttributes() const { return left_attributes_; }

  const std::vector<const AttributeInfo *> &GetRightAttributes() const { return right_attributes_; }
 ...
 protected:
  ...
  virtual void HandleSubplanBinding(bool from_left, const BindingContext &input) = 0;
