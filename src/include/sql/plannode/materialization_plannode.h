...
class MaterializationPlan : public AbstractPlan {
 public:
  ...
  MaterializationPlan(bool physify_flag) : schema_(nullptr), physify_flag_(physify_flag) {}

  inline const std::unordered_map<oid_t, oid_t> &GetOldToNewCols() const { return old_to_new_cols_; }
  ...
  std::unique_ptr<AbstractPlan> Copy() const {
    std::shared_ptr<const catalog::Schema> schema_copy(catalog::Schema::CopySchema(schema_));
    return std::unique_ptr<AbstractPlan>(new MaterializationPlan(old_to_new_cols_, schema_copy, physify_flag_));
  }
