...
class AnalyzePlan : public AbstractPlan {
 public:
  ...
  explicit AnalyzePlan(std::string table_name, std::string schema_name, std::string database_name,
                       concurrency::TransactionContext *txn);

  explicit AnalyzePlan(std::string table_name, std::string schema_name, std::string database_name,
                       std::vector<char *> column_names, concurrency::TransactionContext *txn);

  explicit AnalyzePlan(parser::AnalyzeStatement *parse_tree, concurrency::TransactionContext *txn);
