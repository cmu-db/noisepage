...
class SeqScanPlan : public AbstractScan {
 public:
  ...
  // Normal plan
  SeqScanPlan(storage::DataTable *table, expression::AbstractExpression *predicate,
              const std::vector<oid_t> &column_ids, bool is_for_update = false, bool parallel = false)
  ...
  const std::string GetInfo() const override { return "SeqScanPlan(" + GetPredicateInfo() + ")"; }
  ...
  std::unique_ptr<AbstractPlan> Copy() const override {
    auto *new_plan = new SeqScanPlan(GetTable(), GetPredicate()->Copy(), GetColumnIds());
