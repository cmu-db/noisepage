...
class AggregatePlan : public AbstractPlan {
 public:
  ...
  AggregatePlan(std::unique_ptr<const planner::ProjectInfo> &&project_info,
                ...
                const std::vector<AggTerm> &&unique_agg_terms, const std::vector<oid_t> &&groupby_col_ids,
                std::shared_ptr<const catalog::Schema> &output_schema, AggregateType aggregate_strategy)
  ...
  const std::vector<oid_t> &GetGroupbyColIds() const { return groupby_col_ids_; }

  const std::vector<const AttributeInfo *> &GetGroupbyAIs() const { return groupby_ais_; }
  ...
  const std::vector<AggTerm> &GetUniqueAggTerms() const { return unique_agg_terms_; }

  const catalog::Schema *GetOutputSchema() const { return output_schema_.get(); }

  AggregateType GetAggregateStrategy() const { return agg_strategy_; }

  inline PlanNodeType GetPlanNodeType() const override { return PlanNodeType::AGGREGATE_V2; }
  ...
  const std::string GetInfo() const override {
    return "AggregatePlan(Having(" + (predicate_ != nullptr ? predicate_->GetInfo() : "") + "))";
  }
  ...
  std::unique_ptr<AbstractPlan> Copy() const override {
    ...
    std::shared_ptr<const catalog::Schema> output_schema_copy(catalog::Schema::CopySchema(GetOutputSchema()));
    AggregatePlan *new_plan = new AggregatePlan(
        ...
        std::move(copied_agg_terms), std::move(copied_groupby_col_ids), output_schema_copy, agg_strategy_);
 ...
 private:
  ...
  hash_t Hash(const std::vector<planner::AggregatePlan::AggTerm> &agg_terms) const;
