...
class OrderByPlan : public AbstractPlan {
 public:
  ...
  OrderByPlan(const std::vector<oid_t> &sort_keys, const std::vector<bool> &descend_flags,
              const std::vector<oid_t> &output_column_ids);

  OrderByPlan(const std::vector<oid_t> &sort_keys, const std::vector<bool> &descend_flags,
              const std::vector<oid_t> &output_column_ids, uint64_t limit, uint64_t offset);
  ...
  const std::vector<const planner::AttributeInfo *> &GetSortKeyAIs() const { return sort_key_ais_; }

  const std::vector<bool> &GetDescendFlags() const { return descend_flags_; }

  const std::vector<oid_t> &GetOutputColumnIds() const { return output_column_ids_; }

  const std::vector<const planner::AttributeInfo *> &GetOutputColumnAIs() const { return output_ais_; }

  PlanNodeType GetPlanNodeType() const override { return PlanNodeType::ORDERBY; }

  void GetOutputColumns(std::vector<oid_t> &columns) const override { columns = GetOutputColumnIds(); }
