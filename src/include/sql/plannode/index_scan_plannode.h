...
class IndexScanPlan : public AbstractScan {
 public:
  ...
  struct IndexScanDesc {
     ...
     */
    IndexScanDesc(oid_t p_index_id, const std::vector<oid_t> &p_tuple_column_id_list,
                  const std::vector<ExpressionType> &expr_list_p, const std::vector<type::Value> &p_value_list,
  ...
  IndexScanPlan(storage::DataTable *table, expression::AbstractExpression *predicate,
                const std::vector<oid_t> &column_ids, const IndexScanDesc &index_scan_desc,
  ...
  inline PlanNodeType GetPlanNodeType() const { return PlanNodeType::INDEXSCAN; }
  ...
  std::unique_ptr<AbstractPlan> Copy() const {
    ...
    IndexScanDesc desc(index_id_, key_column_ids_, expr_types_, values_, new_runtime_keys);
    IndexScanPlan *new_plan = new IndexScanPlan(GetTable(), GetPredicate()->Copy(), GetColumnIds(), desc, false);
