...
class NestedLoopJoinPlan : public AbstractJoinPlan {
 public:
  NestedLoopJoinPlan(JoinType join_type, std::unique_ptr<const expression::AbstractExpression> &&predicate,
                     std::unique_ptr<const ProjectInfo> &&proj_info,
                     std::shared_ptr<const catalog::Schema> &proj_schema,
                     const std::vector<oid_t> &join_column_ids_left, const std::vector<oid_t> &join_column_ids_right);
  ...
  const std::vector<oid_t> &GetJoinColumnsLeft() const { return join_column_ids_left_; }

  const std::vector<const planner::AttributeInfo *> GetJoinAIsLeft() const { return join_ais_left_; }

  const std::vector<oid_t> &GetJoinColumnsRight() const { return join_column_ids_right_; }

  const std::vector<const planner::AttributeInfo *> GetJoinAIsRight() const { return join_ais_right_; }
