...
class MergeJoinPlan : public AbstractJoinPlan {
 public:
  struct JoinClause {
    JoinClause(const expression::AbstractExpression *left, const expression::AbstractExpression *right, bool reversed)
  ...
  MergeJoinPlan(JoinType join_type, std::unique_ptr<const expression::AbstractExpression> &&predicate,
                std::unique_ptr<const ProjectInfo> &&proj_info, std::shared_ptr<const catalog::Schema> &proj_schema,
  ...
  void HandleSubplanBinding(bool from_left, const BindingContext &input) override {
    for (auto &join_clause : *GetJoinClauses()) {
      auto &exp = from_left ? join_clause.left_ : join_clause.right_;
      const_cast<expression::AbstractExpression *>(exp.get())->PerformBinding({&input});
    }
  }
  ...
  std::unique_ptr<AbstractPlan> Copy() const override {
    ...
    for (size_t i = 0; i < join_clauses_.size(); i++) {
      new_join_clauses.push_back(
          JoinClause(join_clauses_[i].left_->Copy(), join_clauses_[i].right_->Copy(), join_clauses_[i].reversed_));
    }

    std::unique_ptr<const expression::AbstractExpression> predicate_copy(GetPredicate()->Copy());
    std::shared_ptr<const catalog::Schema> schema_copy(catalog::Schema::CopySchema(GetSchema()));
    MergeJoinPlan *new_plan = new MergeJoinPlan(GetJoinType(), std::move(predicate_copy), GetProjInfo()->Copy(),
                                                schema_copy, new_join_clauses);
