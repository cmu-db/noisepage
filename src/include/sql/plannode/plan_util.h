...
class PlanUtil {
 public:
   ...
   */
  static const std::set<oid_t> GetTablesReferenced(const planner::AbstractPlan *plan);
   ...
   */
  static const std::set<oid_t> GetAffectedIndexes(catalog::CatalogCache &catalog_cache,
   ...
   */
  static const std::vector<col_triplet> GetIndexableColumns(catalog::CatalogCache &catalog_cache,
 ...
 private:
  ...
  static void GetInfo(const planner::AbstractPlan *plan, std::ostringstream &os, int num_indent);

  static void GetTablesReferenced(const planner::AbstractPlan *plan, std::set<oid_t> &table_ids);
};
...
inline void PlanUtil::GetInfo(const planner::AbstractPlan *plan, std::ostringstream &os, int num_indent) {
...
inline const std::set<oid_t> PlanUtil::GetTablesReferenced(const planner::AbstractPlan *plan) {
...
inline void PlanUtil::GetTablesReferenced(const planner::AbstractPlan *plan, std::set<oid_t> &table_ids) {
  switch (plan->GetPlanNodeType()) {
    case PlanNodeType::SEQSCAN:
    case PlanNodeType::INDEXSCAN: {
      const auto *scan_node = reinterpret_cast<const planner::AbstractScan *>(plan);
      table_ids.insert(scan_node->GetTable()->GetOid());
      break;
    }
    case PlanNodeType::INSERT: {
      const auto *insert_node = reinterpret_cast<const planner::InsertPlan *>(plan);
      table_ids.insert(insert_node->GetTable()->GetOid());
      break;
    }
    case PlanNodeType::UPDATE: {
      const auto *update_node = reinterpret_cast<const planner::UpdatePlan *>(plan);
      table_ids.insert(update_node->GetTable()->GetOid());
      break;
    }
    case PlanNodeType::DELETE: {
      const auto *delete_node = reinterpret_cast<const planner::DeletePlan *>(plan);
      table_ids.insert(delete_node->GetTable()->GetOid());
      break;
    }
    case PlanNodeType::POPULATE_INDEX: {
      const auto *populate_index_node = reinterpret_cast<const planner::PopulateIndexPlan *>(plan);
