...
class HashJoinPlan : public AbstractJoinPlan {
 ...
 public:
  HashJoinPlan(JoinType join_type, ExpressionPtr &&predicate, std::unique_ptr<const ProjectInfo> &&proj_info,
               std::shared_ptr<const catalog::Schema> &proj_schema, std::vector<ExpressionPtr> &left_hash_keys,
               std::vector<ExpressionPtr> &right_hash_keys, bool build_bloomfilter = false);
  ...
  void GetLeftHashKeys(std::vector<const expression::AbstractExpression *> &keys) const;

  void GetRightHashKeys(std::vector<const expression::AbstractExpression *> &keys) const;
