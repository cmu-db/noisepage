...
class ProjectInfo {
 public:
  ...
  ProjectInfo(TargetList &&tl, DirectMapList &&dml) : target_list_(tl), direct_map_list_(dml) {}

  ~ProjectInfo();

  void PerformRebinding(BindingContext &output_context,
  ...
  bool Evaluate(storage::Tuple *dest, const AbstractTuple *tuple1, const AbstractTuple *tuple2,
                executor::ExecutorContext *econtext) const;

  bool Evaluate(AbstractTuple *dest, const AbstractTuple *tuple1, const AbstractTuple *tuple2,
                executor::ExecutorContext *econtext) const;
  ...
  std::unique_ptr<const ProjectInfo> Copy() const {
    ...
    for (const Target &target : target_list_) {
      new_target_list.emplace_back(target.first, DerivedAttribute{target.second.expr->Copy()});
    }
    ...
    for (const DirectMap &aMap : direct_map_list_) {
      new_map_list.push_back(std::pair<oid_t, std::pair<oid_t, oid_t>>(
          aMap.first, std::pair<oid_t, oid_t>(aMap.second.first, aMap.second.second)));
    }

    return std::unique_ptr<ProjectInfo>(new ProjectInfo(std::move(new_target_list), std::move(new_map_list)));
  }
 ...
 private:
  bool AreEqual(const planner::DerivedAttribute &A, const planner::DerivedAttribute &B) const;
