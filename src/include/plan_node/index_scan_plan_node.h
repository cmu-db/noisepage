...
class IndexScanPlanNode : public AbstractScanPlanNode {
 public:
  IndexScanPlanNode(catalog::Schema output_schema, catalog::index_oid_t index_oid,
                    parser::AbstractExpression *predicate, bool is_for_update = false)
      : AbstractScanPlanNode(output_schema, predicate, is_for_update), index_oid_(index_oid) {}
  ...
  std::unique_ptr<AbstractPlanNode> Copy() const override {
    IndexScanPlanNode *new_plan =
        new IndexScanPlanNode(GetOutputSchema(), GetIndexOid(), GetPredicate()->Copy().get(), IsForUpdate());
