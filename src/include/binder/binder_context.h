...
class BinderContext {
 public:
  ...
  /**
   * Generate list of column value expression that covers all columns of a certain table
   * @param table_name Table to generate columns for
   * @param parse_result Result generated by the parser. A collection of statements and expressions in the query.
   * @param exprs Pointer to the list of column value expression.
   * The generated column value expressions will be placed in this list.
   */
  void GenerateColumnExpressions(const std::string &table_name, parser::ParseResult *parse_result,
                                 std::vector<common::ManagedPointer<parser::AbstractExpression>> *exprs);
