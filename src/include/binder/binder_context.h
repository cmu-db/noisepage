...
class BinderContext {
 public:
   ...
   * clause)
   */
  void AddRegularTable(catalog::CatalogAccessor *accessor, const std::string &db_name, const std::string &table_name, const std::string &table_alias);
  ...
  static void GetColumnPosTuple(const std::string &col_name, std::tuple<catalog::db_oid_t, catalog::table_oid_t, catalog::Schema> tuple, parser::ColumnValueExpression *expr);
...
//  std::unordered_map<std::string, std::shared_ptr<catalog::Schema>>
