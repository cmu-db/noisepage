...
class Catalog {
 ...
 public:
  ...
  index_oid_t GetIndexOid(namespace_oid_t ns, const std::string &name) {
    auto iter1 = namespace_index_map_.find(ns);
    if (iter1 == namespace_index_map_.end()) return INVALID_INDEX_OID;

    auto iter2 = iter1->second.find(name);
    return (iter2 == iter1->second.end()) ? INVALID_INDEX_OID : iter2->second;
  }
  ...
  std::vector<index_oid_t> GetIndexOids(table_oid_t table) {
    auto iter = table_index_map_.find(table);
  ...
  index_oid_t CreateIndex(namespace_oid_t ns, table_oid_t table, std::string name,
    ...
    namespace_index_map_[ns][name] = idx_oid;
    table_index_map_[table].insert(idx_oid);
    index_to_name_map_[idx_oid] = name;
    index_to_namespace_map_[idx_oid] = ns;
    index_to_table_map_[idx_oid] = table;
    index_to_key_schema_map_[idx_oid] = std::move(schema);
  ...
  bool DropIndex(index_oid_t index) {
    if (index_to_name_map_.find(index) == index_to_name_map_.end()) return false;
    auto table = index_to_table_map_[index];
    auto ns = index_to_namespace_map_[index];
    auto name = index_to_name_map_[index];

    // Delete the "index" from all seven maps...
    table_index_map_[table].erase(index);
    namespace_index_map_[ns].erase(name);
    index_to_name_map_.erase(index);
    index_to_table_map_.erase(index);
    index_to_namespace_map_.erase(index);
    index_to_pointer_map_.erase(index);
    index_to_key_schema_map_.erase(index);
    return true;
  }
  ...
  bool SetIndexPointer(index_oid_t index, storage::index::Index *index_ptr) {
    if (GetIndex(index) != nullptr) return false;

    index_to_pointer_map_[index] = index_ptr;
    return true;
  }
  ...
  storage::index::Index *GetIndex(index_oid_t index) {
    auto iter = index_to_pointer_map_.find(index);
    return (iter == index_to_pointer_map_.end()) ? nullptr : iter->second;
  }
 ...
 private:
  ...
  // The following maps only exist to allow the catalog API to be used for end-to-end testing
  // of simple scenarios.  They are NOT thread safe or transactional if modifications are made
  // to the data (insert, delete, updates).  Therefore, we only support the following scenario:
  // (Single threaded) Entire schema, to include indexes, created
  // (Multi-threaded) DML transactions that only do catalog lookups
  // (Single threaded) Teardown of catalog
  std::unordered_map<namespace_oid_t, std::unordered_map<std::string, index_oid_t>> namespace_index_map_;
  std::unordered_map<table_oid_t, std::unordered_set<index_oid_t>> table_index_map_;
  std::unordered_map<index_oid_t, table_oid_t> index_to_table_map_;
  std::unordered_map<index_oid_t, namespace_oid_t> index_to_namespace_map_;
  std::unordered_map<index_oid_t, storage::index::Index *> index_to_pointer_map_;
  std::unordered_map<index_oid_t, IndexKeySchema> index_to_key_schema_map_;
  std::unordered_map<index_oid_t, std::string> index_to_name_map_;
