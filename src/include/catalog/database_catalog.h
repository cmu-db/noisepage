...
class DatabaseCatalog {
 public:
  ...
  void CopyColumnData(storage::ProjectedRow *table_pr, storage::ProjectedRow *index_pr, std::vector<col_oid_t> col_vec, const Schema &schema);
  bool VerifyUniquePKConstraint(common::ManagedPointer<transaction::TransactionContext> txn, const PG_Constraint &con_obj, storage::ProjectedRow *pr);
  ...
  std::vector<constraint_oid_t> CreateFKConstraintInFKTable(common::ManagedPointer<transaction::TransactionContext> txn,
                                   constraint_oid_t constraint, table_oid_t src_table, table_oid_t sink_table,
